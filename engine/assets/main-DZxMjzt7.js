...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${c}`);let y=!1;const L=un(s,!0);r!=="folded"&&e!==P.BLOCK_FOLDED&&(L.onOverflow=()=>{y=!0});const S=cn(`${v}${k}${h}`,c,qn,L);if(!y)return`>${E}
...
`+E.comment:N.comment=E.comment);const k=new be(N);t.options.keepSourceTokens&&(k.srcToken=d),o.items.push(k)}}return c&&c<l&&i(c,"IMPOSSIBLE","Map comment with trailing content"),o.range=[s.offset,l,c??l],o}function er({composeNode:n,composeEmptyNode:e},t,s,i,a){const r=(a==null?void 0:a.nodeClass)??nt,o=new r(t.schema);t.atRoot&&(t.atRoot=!1),t.atKey&&(t.atKey=!1);let l=s.offset,c=null;for(const{start:u,value:d}of s.items){const m=bt(u,{indicator:"seq-item-ind",next:d,offset:l,onError:i,parentIndent:s.indent,startOnNewline:!0});if(!m.found)if(m.anchor||m.tag||d)(d==null?void 0:d.type)==="block-seq"?i(m.end,"BAD_INDENT","All sequence items must start at the same column"):i(l,"MISSING_CHAR","Sequence item without - indicator");else{c=m.end,m.comment&&(o.comment=m.comment);continue}const h=d?n(t,d,m,i):e(t,m.end,u,null,m,i);t.schema.compat&&Yn(s.indent,d,i),l=h.range[2],o.items.push(h)}return o.range=[s.offset,l,c??l],o}function Dt(n,e,t,s){let i="";if(n){let a=!1,r="";for(const o of n){const{source:l,type:c}=o;switch(c){case"space":a=!0;break;case"comment":{t&&!a&&s(o,"MISSING_CHAR","Comments must be separated from other tokens by white space characters");const u=l.substring(1)||" ";i?i+=r+u:i=u,r="";break}case"newline":i&&(r+=l),a=!0;break;default:s(o,"UNEXPECTED_TOKEN",`Unexpected ${c} at node end`)}e+=l.length}}return{comment:i,offset:e}}const Cn="Block collections are not allowed within flow collections",An=n=>n&&(n.type==="block-map"||n.type==="block-seq");function tr({composeNode:n,composeEmptyNode:e},t,s,i,a){var g;const r=s.start.source==="{",o=r?"flow map":"flow sequence",l=(a==null?void 0:a.nodeClass)??(r?Te:nt),c=new l(t.schema);c.flow=!0;const u=t.atRoot;u&&(t.atRoot=!1),t.atKey&&(t.atKey=!1);let d=s.offset+s.start.source.length;for(let _=0;_<s.items.length;++_){const v=s.items[_],{start:N,key:E,sep:k,value:y}=v,L=bt(N,{flow:o,indicator:"explicit-key-ind",next:E??(k==null?void 0:k[0]),offset:d,onError:i,parentIndent:s.indent,startOnNewline:!1});if(!L.found){if(!L.anchor&&!L.tag&&!k&&!y){_===0&&L.comma?i(L.comma,"UNEXPECTED_TOKEN",`Unexpected , in ${o}`):_<s.items.length-1&&i(L.start,"UNEXPECTED_TOKEN",`Unexpected empty item in ${o}`),L.comment&&(c.comment?c.comment+=`
`+L.comment:c.comment=L.comment),d=L.end;continue}!r&&t.options.strict&&$t(E)&&i(E,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line")}if(_===0)L.comma&&i(L.comma,"UNEXPECTED_TOKEN",`Unexpected , in ${o}`);else if(L.comma||i(L.start,"MISSING_CHAR",`Missing , between ${o} items`),L.comment){let S="";e:for(const M of N)switch(M.type){case"comma":case"space":break;case"comment":S=M.source.substring(1);break e;default:break e}if(S){let M=c.items[c.items.length-1];te(M)&&(M=M.value??M.key),M.comment?M.comment+=`
`+S:M.comment=S,L.comment=L.comment.substring(S.length+1)}}if(!r&&!k&&!L.found){const S=y?n(t,y,L,i):e(t,L.end,k,null,L,i);c.items.push(S),d=S.range[2],An(y)&&i(S.range,"BLOCK_IN_FLOW",Cn)}else{t.atKey=!0;const S=L.end,M=E?n(t,E,L,i):e(t,S,N,null,L,i);An(E)&&i(M.range,"BLOCK_IN_FLOW",Cn),t.atKey=!1;const $=bt(k??[],{flow:o,indicator:"map-value-ind",next:y,offset:M.range[2],onError:i,parentIndent:s.indent,startOnNewline:!1});if($.found){if(!r&&!L.found&&t.options.strict){if(k)for(const R of k){if(R===$.found)break;if(R.type==="newline"){i(R,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line");break}}L.start<$.found.offset-1024&&i($.found,"KEY_OVER_1024_CHARS","The : indicator must be at most 1024 chars after the start of an implicit flow sequence key")}}else y&&("source"in y&&((g=y.source)==null?void 0:g[0])===":"?i(y,"MISSING_CHAR",`Missing space after : in ${o}`):i($.start,"MISSING_CHAR",`Missing , or : between ${o} items`));const x=y?n(t,y,$,i):$.found?e(t,$.end,k,null,$,i):null;x?An(y)&&i(x.range,"BLOCK_IN_FLOW",Cn):$.comment&&(M.comment?M.comment+=`
`+$.comment:M.comment=$.comment);const U=new be(M,x);if(t.options.keepSourceTokens&&(U.srcToken=v),r){const R=c;_i(t,R.items,M)&&i(S,"DUPLICATE_KEY","Map keys must be unique"),R.items.push(U)}else{const R=new Te(t.schema);R.flow=!0,R.items.push(U);const ie=(x??M).range;R.range=[M.range[0],ie[1],ie[2]],c.items.push(R)}d=x?x.range[2]:$.end}}const m=r?"}":"]",[h,...b]=s.end;let p=d;if((h==null?void 0:h.source)===m)p=h.offset+h.source.length;else{const _=o[0].toUpperCase()+o.substring(1),v=u?`${_} must end with a ${m}`:`${_} in block collection must be sufficiently indented and end with a ${m}`;i(d,u?"MISSING_CHAR":"BAD_INDENT",v),h&&h.source.length!==1&&b.unshift(h)}if(b.length>0){const _=Dt(b,p,t.options.strict,i);_.comment&&(c.comment?c.comment+=`
...
`;function Q(n){if(/^[a-z]+:\/\//i.test(n)||n.startsWith("data:")||n.startsWith("blob:"))return n;const e="/engine/",t=e.endsWith("/")?e:`${e}/`,s=n.startsWith("/")?n.slice(1):n;return`${t}${s}`}function Ai(n,e,t){const s=n.indexOf("#"),i=s>=0?n.slice(0,s):n,a=s>=0?n.slice(s):"",r=new RegExp(`([?&])${Or(e)}=[^&#]*`);if(r.test(i))return`${i.replace(r,`$1${e}=${encodeURIComponent(t)}`)}${a}`;const o=i.includes("?")?"&":"?";return`${i}${o}${e}=${encodeURIComponent(t)}${a}`}function Or(n){return n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}const Pr=We(Cr),xr=We(Ar),On=We(Tr),$r=We(Mr),ct=Object.entries(Pr).map(([n,e])=>{var r,o,l;const t=Rr(On[n]),s=(((r=On[n])==null?void 0:r.results)??[]).map(Fr),i=((o=$r[n])==null?void 0:o.liveStats)??[],a=xr[n]??{};return{id:n,label:e.label,placeholderImage:Q(e.placeholderImage),parameterSubtitle:e.parameterSubtitle,metadata:{results:s,summaryStats:t.map(zn),liveStats:i.map(zn),morphologyChecklist:(l=On[n])==null?void 0:l.morphologyChecklist},parameters:Object.entries(a).map(([c,u])=>{const d=u.quali_labels,m=d!==void 0&&d.length>0,h=m?0:u.min,b=m?d.length-1:u.max,p=m?1:u.step??Br(u.min,u.max),g=m?Math.floor(d.length/2):u.log_scale?Math.sqrt(u.min*u.max):Vr(u.min,u.max);return{id:c,label:u.label,unit:u.unit??"",min:h,max:b,step:p,fallbackValue:g,description:u.description,valueScale:u.value_scale,displayUnit:u.display_unit,displayFormat:u.display_format,displaySignificantFigures:u.display_significant_figures,logScale:u.log_scale,qualiLabels:d,primary:u.primary??!0}}),views:(e.views??[]).map(c=>({id:c.id,label:c.label,icon:c.icon,audio:c.audio??!1,description:c.description}))}});function Rr(n){if(!n)return[];const e=[];for(const t of n.resources??[])e.push({...t,section:"resources"});for(const t of n.simulationStats??[])e.push({...t,section:"simulationStats"});return n.similarityScore&&e.push({id:"similarityScore",value:n.similarityScore.value}),e}function zn(n){return{id:n.id,label:n.label,section:n.section,value:n.value,unit:n.unit,description:n.description,live:n.live??!1,liveKey:n.live_key,fromVideo:n.from_video??!1,videoKey:n.video_key,scaleWithTime:n.scale_with_time??!1,integer:n.integer??!1,valueScale:n.value_scale,displayFormat:n.display_format,precision:n.precision}}function Fr(n){return{...zn(n),target:n.target}}function Br(n,e){const s=Math.max(e-n,1e-9)/100,i=10**Math.floor(Math.log10(s)),a=s/i;let r=1;return a<=1?r=1:a<=2?r=2:a<=5?r=5:r=10,r*i}function Vr(n,e){return n+(e-n)/2}const Ti="universe-engine-theme",Mi=[{id:"glass",label:"Glass",icon:"[ ]"},{id:"matrix",label:"Matrix",icon:"[#]"},{id:"hal",label:"HAL 9000",icon:"( )"},{id:"nostromo",label:"Nostromo",icon:"[=]"},{id:"tron",label:"Tron",icon:"<>"}];function Ur(){const n=localStorage.getItem(Ti);return Hr(n)?n:"glass"}function Pn(n){document.documentElement.setAttribute("data-theme",n),localStorage.setItem(Ti,n)}function Dr(n,e,t){const s=document.createElement("div");s.className="theme-picker";const i=new Map;for(const r of Mi){const o=document.createElement("button");o.className="theme-picker__option",o.type="button",o.innerHTML=`
  ...
  `;const o=document.createElement("div");o.className="entry-overlay__actions";function l(m){o.innerHTML="";for(const h of m){const b=document.createElement("button");b.className="entry-overlay__button",b.type="button";const p=oo[h.id]??"Explore this simulation scale.";b.innerHTML=`
      ...
      `,b.addEventListener("click",()=>t(h)),o.appendChild(b)}}l(e);const{infoButton:c,infoModal:u,close:d}=io();return r.appendChild(o),a.appendChild(r),a.appendChild(c),a.appendChild(u),n.appendChild(a),{show(){a.hidden=!1,a.classList.remove("is-hidden")},hide(){d(),a.hidden=!0,a.classList.add("is-hidden")},setSimulationClasses(m){l(m)}}}function co(n,e,t,s){const i=Object.fromEntries(n.metadata.results.map(E=>[E.id,E.target])),a=n.metadata.results.map(E=>{const k=Rn(n,e,s,E.id);return k===null?null:{id:E.id,value:k,target:E.target}}).filter(E=>E!==null),r=n.parameters.filter(E=>i[E.id]!==void 0).map(E=>{const k=e[E.id]??E.fallbackValue,y=i[E.id]??E.fallbackValue;return Math.abs(k-y)/Math.max(E.max-E.min,1e-9)}),o=r.reduce((E,k)=>E+k,0)/Math.max(r.length,1),l=ho(a),c=((s==null?void 0:s.carbonBurnt)??.8+o*4.2).toFixed(2),u=(s==null?void 0:s.computeUsed)??18+o*46,d=(s==null?void 0:s.memoryUsed)??12+o*84,m=`${$n(u,1)} CPU-hrs
...
`,Vs=(()=>{const n=We(go),e={};for(const t of Object.values(n))for(const[s,i]of Object.entries(t))e[s]=i;return e})(),rn="#4CD98A",Qn="#E8951C",$i="#D7372A",Ri=.2,Fi=.5,Us=2;function Bi(n){const e=Math.abs(n-1);return e<=Ri?{word:"On target",colour:rn}:e<=Fi?{word:n>1?"Too high":"Too low",colour:Qn}:{word:n>1?"Way too high":"Way too low",colour:$i}}function yo(n){const e=Math.abs(n-1),t=n>=1;return e<=Ri?t?"greenHigh":"greenLow":e<=Fi?t?"amberHigh":"amberLow":t?"redHigh":"redLow"}function bo(n){return Math.min(Math.max(n,0),Us)/Us*100}function wo(n){return n>=85?{word:"Almost perfect",colour:rn}:n>=65?{word:"Really close",colour:rn}:n>=45?{word:"Getting there",colour:Qn}:n>=25?{word:"Not quite",colour:Qn}:{word:"Way off - try again",colour:$i}}function vo(n,e,t){var r,o;const s=yo(t),i=((r=Vs[n])==null?void 0:r[s])??((o=Vs[e])==null?void 0:o[s]);return i||(Bi(t).colour===rn?`${e} is very close to the target value for this simulation.`:t<1?`${e} is below the target value for this simulation.`:`${e} is above the target value for this simulation.`)}function _o(n,e,t){return n.metadata.results.map(s=>{const i=So(s,n,e,t);if(i===null)return null;const a=i/Math.max(s.target,1e-9),r=ko(s,n,t),o=vo(s.id,r,a),l=Fe(Vi(String(i),s),s.unit);return{id:s.id,label:r,value:a,rawValue:i,formattedValue:l,detail:o}}).filter(s=>s!==null)}function So(n,e,t,s){var l;const i=n.id,a=e.parameters.find(c=>c.id===i);if(a)return t[i]??a.fallbackValue;const r=Eo((l=s==null?void 0:s.summaryMetrics[i])==null?void 0:l.value);if(r!==null)return r;const o=s==null?void 0:s.parameterValues[i];return typeof o=="number"&&Number.isFinite(o)?o:null}function ko(n,e,t){var i,a;const s=n.id;return n.label??((i=e.parameters.find(r=>r.id===s))==null?void 0:i.label)??((a=t==null?void 0:t.summaryMetrics[s])==null?void 0:a.label)??s}function Eo(n){if(n===void 0)return null;const e=Number(n);return Number.isFinite(e)?e:null}function Lo(n){if(n.length===0)return 0;const e=n.reduce((t,s)=>t+Math.max(0,1-Math.abs(s.value-1)),0);return Math.round(e/n.length*100)}function No(n,e){const t=document.createElement("section");t.className="overlay overlay--summary",t.hidden=!0,t.classList.add("is-hidden");let s;const i=document.createElement("div");i.className="summary-overlay";const a=document.createElement("div"),r=document.createElement("div");a.className="summary-overlay__header",r.className="summary-overlay__content";const o=document.createElement("p");o.className="summary-overlay__title",o.textContent="Run Summary";const l=document.createElement("p");l.className="summary-overlay__hint",l.textContent="Select any card for more details",a.appendChild(o),a.appendChild(l);const c=document.createElement("div");c.className="summary-overlay__actions";const u=document.createElement("button");u.className="summary-overlay__button",u.type="button",u.textContent="Continue Visualising";const d=document.createElement("button"),m=document.createElement("button");d.className="summary-overlay__button summary-overlay__button--primary",d.type="button",d.textContent="New Parameters",m.className="summary-overlay__button",m.type="button",m.textContent="Home",m.hidden=!e.showHome,u.addEventListener("click",e.onReplay),d.addEventListener("click",e.onParameters),m.addEventListener("click",e.onHome),c.appendChild(d),c.appendChild(u),c.appendChild(m),i.appendChild(a),i.appendChild(r),i.appendChild(c),t.appendChild(i);const h=document.createElement("div");h.className="sci-modal is-hidden",h.innerHTML=`
  ...
  `,t.appendChild(h),n.appendChild(t);const b=h.querySelector(".sci-modal__title"),p=h.querySelector(".sci-modal__verdict"),g=h.querySelector(".sci-modal__body"),_=h.querySelector(".sci-modal__close"),v=new Set;let N=!1;function E(S){const M=Bi(S.value);b.textContent=S.label,p.textContent=M.word,p.style.color=M.colour,p.hidden=!1,g.textContent=S.detail,h.classList.remove("is-hidden")}function k(S,M){b.textContent=S,p.hidden=!0,g.textContent=M,h.classList.remove("is-hidden")}function y(){h.classList.add("is-hidden")}_.addEventListener("click",y),h.addEventListener("click",S=>{S.target===h&&y()}),t.addEventListener("click",S=>{S.target===t&&e.onReplay()});function L(S,M){const $=document.createElement("div");$.className=`${S.className} panel`,$.innerHTML=`<p class="sci-section__title">${S.title}</p>`;const x=document.createElement("div"),U=S.singleRow?Math.max(1,S.stats.length):Math.max(1,Math.min(S.stats.length,S.maxColumns));x.className="metric-grid",S.singleRow&&x.classList.add("metric-grid--single-row"),x.style.setProperty("--summary-grid-columns",String(U)),x.style.setProperty("--summary-grid-max-width",`${S.maxWidthRem}rem`);for(const R of S.stats){const ie=Io(R,M),W=document.createElement("div"),ne=document.createElement("span"),D=document.createElement("span");W.className="res-card",ne.className="res-card__label",ne.textContent=ie.label,D.className="res-card__value",D.textContent=ie.value,W.appendChild(ne),W.appendChild(D),R.description&&(W.classList.add("res-card--has-info"),W.addEventListener("click",()=>{k(ie.label,R.description)})),x.appendChild(W)}return $.appendChild(x),$}return{show(){s&&(window.clearTimeout(s),s=void 0),t.hidden=!1,t.classList.remove("is-hidden"),t.classList.remove("is-visible"),t.offsetWidth,requestAnimationFrame(()=>{t.classList.add("is-visible")})},hide(){t.classList.remove("is-visible"),s=window.setTimeout(()=>{t.hidden=!0,t.classList.add("is-hidden"),s=void 0,N&&(v.clear(),N=!1)},mo.HIDE_AFTER_MS)},setHomeVisible(S){m.hidden=!S},update(S,M,$,x,U){var I,T,H,K,F,X;r.innerHTML="",y();const R=co(S,M,$,x),ie=S.metadata.summaryStats,W=_o(S,M,x),ne=new Set(W.map(V=>V.id));let D;if(W.length>0)D=Lo(W);else{const V=((I=R.similarityScore)==null?void 0:I.value)??"0/100";D=parseInt(V,10)||0}const ue=wo(D),_e=document.createElement("div"),Ne=document.createElement("div"),ae=document.createElement("div");_e.className="sci-top",Ne.className="summary-main-column",ae.className="summary-side-column";const re=document.createElement("div");re.className="sci-hero panel",U?(re.classList.add("sci-hero--thumbnail"),re.innerHTML=`<img class="sci-hero__thumbnail" src="${U}" alt="Final frame of simulation" />`):re.innerHTML=`
        ...
        `,Ne.appendChild(re);const Pe=ie.filter(V=>(V.section??"resources")==="resources"&&!W.some(oe=>oe.id===String(V.id))&&V.id!=="similarityScore"),de=ie.filter(V=>V.section==="simulationStats"&&!ne.has(String(V.id)));Pe.length>0&&ae.appendChild(L({title:"Resources Used",className:"res-section",stats:Pe,maxColumns:3,maxWidthRem:48},R)),de.length>0&&ae.appendChild(L({title:"Simulation Stats",className:"res-section",stats:de,maxColumns:de.length,maxWidthRem:48,singleRow:!0},R)),_e.appendChild(Ne),ae.childElementCount>0&&_e.appendChild(ae),r.appendChild(_e);const fe=S.metadata.morphologyChecklist;if(fe&&fe.length>0){const V=((H=(T=x==null?void 0:x.summaryMetrics)==null?void 0:T.morphology)==null?void 0:H.value)??null,oe=(V==null?void 0:V.toLowerCase())??null,Se=new Set(fe.map(B=>B.id));oe&&Se.has(oe)&&v.add(oe);const se=document.createElement("div");se.className="sci-bottom-row";const j=document.createElement("div");j.style.cssText="display:flex; flex-direction:column; gap:1.25rem;",j.appendChild(Ds(S,M,k));const Ie=((K=fe.find(B=>B.id===oe))==null?void 0:K.label)??V;if(Ie){const B=document.createElement("div");B.className="sci-section panel param-section";const q=document.createElement("p");q.className="sci-section__title",q.textContent="Galaxy Morphology";const ge=document.createElement("div");ge.className="res-card res-card--has-info";const C=document.createElement("span");C.className="res-card__value",C.textContent=Ie,ge.appendChild(C),ge.addEventListener("click",()=>{var Y;return k("Galaxy Morphology",((Y=fe.find(J=>J.id===(V??"").toLowerCase()))==null?void 0:Y.description)??`This galaxy is classified as "${Ie}".`)}),B.appendChild(q),B.appendChild(ge),j.appendChild(B)}se.appendChild(j);const pe=document.createElement("div");pe.style.cssText="flex:1; display:flex; flex-direction:column; gap:1.25rem; min-width:0; min-height:0;";const Ce=((X=(F=x==null?void 0:x.summaryMetrics)==null?void 0:F.description)==null?void 0:X.value)??null;if(Ce){const B=document.createElement("div");B.className="sci-section panel",B.style.cssText="flex:0 1 auto; min-width:0;";const q=document.createElement("p");q.className="sci-section__title",q.textContent="About This Galaxy";const ge=document.createElement("p");ge.className="galaxy-summary__desc-text",ge.textContent=Ce,B.appendChild(q),B.appendChild(ge),pe.appendChild(B)}const we=document.createElement("div");we.className="sci-section panel",we.style.cssText="flex:1; min-height:0;";const ke=document.createElement("p");ke.className="sci-section__title",ke.textContent="Morphology Scavenger Hunt";const He=document.createElement("div");He.className="galaxy-summary__checklist",He.style.cssText="flex:1; align-items:center;";const Ye=fe.every(B=>v.has(B.id));for(const B of fe){const q=document.createElement("div");q.className="galaxy-summary__check",v.has(B.id)&&q.classList.add("is-found"),q.innerHTML=`
            <span class="galaxy-summary__check-box">
              ${v.has(B.id)?"✓":""}
            </span>
            <span class="galaxy-summary__check-label">${B.label}</span>
          `,B.hint&&(q.classList.add("res-card--has-info"),q.addEventListener("click",()=>k(B.label,B.hint))),He.appendChild(q)}if(we.appendChild(ke),we.appendChild(He),Ye){N=!0;const B=document.createElement("div");B.className="galaxy-summary__done",B.textContent="★ You've discovered all of the galaxy types. Well done! ★",we.appendChild(B)}pe.appendChild(we),se.appendChild(pe),r.appendChild(se)}else if(W.length>0){const V=document.createElement("div");V.className="sci-bottom-row",V.appendChild(Ds(S,M,k));const oe=document.createElement("div"),Se=document.createElement("div"),se=document.createElement("p"),j=document.createElement("p");oe.className="sci-section panel",Se.className="sci-section__header",se.className="sci-section__title",se.textContent="Similarity Results",j.className="sci-section__hint",j.textContent="Select any bar for details",Se.appendChild(se),Se.appendChild(j);const Ie=document.createElement("div");Ie.className="sci-bars";for(const pe of W){const Ce=document.createElement("div");Ce.className="sci-bar",Ce.innerHTML=`
      ...
      `);case"icon_temperature":return Le(`
      ...
      `);case"gas-temperature":return Le(`
      ...
      `);case"hubble-space-telescope":return Le(`
      ...
      `);case"large-scale-structure":return Le(`
      ...
      `);default:return null}}function Le(n){const e=document.createElementNS("http://www.w3.org/2000/svg","svg");return e.setAttribute("viewBox","0 0 24 24"),e.setAttribute("fill","none"),e.setAttribute("stroke","currentColor"),e.setAttribute("stroke-width","1.5"),e.setAttribute("stroke-linecap","round"),e.setAttribute("stroke-linejoin","round"),e.innerHTML=n,e}function Mo(){return Le(`
    <circle cx="12" cy="12" r="10"></circle>
    <path d="M12 16.5v-6"></path>
    <circle cx="12" cy="8.5" r="1.1" fill="currentColor" stroke="none"></circle>
...
`;function Po(){const n=We(Oo);if(!Array.isArray(n))return[];const e=[];for(const t of n){if(!t||typeof t!="object")continue;const s=t.text;if(typeof s!="string"||s.trim().length===0)continue;const i={text:s},a=t.url;typeof a=="string"&&a.trim().length>0&&(i.url=a);const r=t.logo;typeof r=="string"&&r.trim().length>0&&(i.logo=r),t.header===!0&&(i.header=!0),e.push(i)}return e}function xo(n,e,t,s){const i=document.createElement("div");i.className="parameter-editor",n.appendChild(i);let a=e,r={...t};function o(u,d){a=u,r=d?{...d}:$o(u),i.innerHTML="";const m=document.createElement("div");m.className="parameter-editor__heading",m.innerHTML=`
 ...
 Engine</h1>
  ...
  `,E.appendChild(k),E.appendChild(y),N.appendChild(E);const L=document.createElement("label");L.className="advanced-settings__field",L.innerHTML=`
  ...
  `;const S=document.createElement("input"),M=document.createElement("span");S.type="range",S.min="0",S.max="100",S.step="1",S.className="audio-settings__slider",M.className="audio-settings__value",L.appendChild(S),L.appendChild(M),N.appendChild(L),_.appendChild(N);const $=document.createElement("section");$.className="advanced-settings",$.dataset.state="closed",$.innerHTML=`
  ...
  `;const x=document.createElement("button");x.className="advanced-settings__access",x.type="button",x.textContent="Advanced Settings",$.appendChild(x);const U=document.createElement("div");U.className="advanced-settings__auth";const R=document.createElement("input");R.className="advanced-settings__password",R.type="password",R.placeholder="Enter password",R.autocomplete="off";const ie=document.createElement("button");ie.className="advanced-settings__unlock",ie.type="button",ie.textContent="Unlock";const W=document.createElement("p");W.className="advanced-settings__message",U.appendChild(R),U.appendChild(ie),U.appendChild(W),$.appendChild(U);const ne=document.createElement("div");ne.className="advanced-settings__form";const D=document.createElement("label");D.className="advanced-settings__field",D.innerHTML=`
  ...
  `;const ue=document.createElement("select");ue.className="advanced-settings__select",ue.appendChild(new Option("None",""));for(const C of e.availableScales)ue.appendChild(new Option(C.label,C.id));D.appendChild(ue),ne.appendChild(D);const _e=document.createElement("div");_e.className="advanced-settings__field",_e.innerHTML=`
  ...
  `,fe.appendChild(I),fe.appendChild(T),ne.appendChild(fe);const H=document.createElement("div");H.className="advanced-settings__field",H.innerHTML=`
  ...
  `;const K=document.createElement("div");K.className="advanced-settings__options";const F=new Map;for(const C of e.availableScales){const Y=document.createElement("label"),J=document.createElement("input");Y.className="advanced-settings__choice",J.type="checkbox",J.value=C.id,F.set(C.id,J),Y.appendChild(J),Y.append(`Show ${C.label}`),K.appendChild(Y)}H.appendChild(K),ne.appendChild(H),$.appendChild(ne),_.appendChild($);const X=document.createElement("section");X.className="config-overlay__section config-overlay__section--grow",X.dataset.section="credits",X.innerHTML=`
    <div class="credits-list" data-credits></div>
  `;const V=X.querySelector("[data-credits]"),oe=Po();if(V.innerHTML="",oe.length===0){const C=document.createElement("div");C.className="credits-list__entry",C.textContent="To be credited...",V.appendChild(C)}else for(const C of oe)if(C.header){const Y=document.createElement("div");Y.className="credits-list__heading",Y.textContent=C.text,V.appendChild(Y)}else{const Y=document.createElement("div");Y.className="credits-list__entry";const J=document.createElement("span");if(J.className="credits-list__text",C.url){const z=document.createElement("a");z.className="credits-list__link",z.href=C.url,z.target="_blank",z.rel="noopener noreferrer",z.textContent=C.text,J.appendChild(z)}else J.textContent=C.text;Y.appendChild(J),V.appendChild(Y)}const Se=document.createElement("div");Se.className="config-overlay__footer";const se=document.createElement("button");se.className="run-button",se.type="button",se.textContent="Run",Se.appendChild(se),l.appendChild(c),l.appendChild(p),l.appendChild(_),l.appendChild(X),l.appendChild(Se),i.appendChild(a),i.appendChild(l),s.appendChild(i),t.appendChild(s),n.appendChild(t);let j=At(e.advancedSettings),Ie="closed";const pe=xo(g,e.simClass,e.values,e.onValuesChange),Ce=Dr(v,e.theme,e.onThemeChange);b.addEventListener("click",e.onClose),x.addEventListener("click",()=>{if(Ie==="open"){Ye("closed");return}Ye("auth"),R.focus()}),ie.addEventListener("click",He),R.addEventListener("keydown",C=>{C.key==="Enter"&&He()}),ue.addEventListener("change",()=>{j.lockedScaleId=ue.value||null,ke()}),re.addEventListener("change",()=>{re.checked&&(j.manifestSource="local")}),de.addEventListener("change",()=>{de.checked&&(j.manifestSource="online")}),I.addEventListener("change",()=>{j.verboseLogging=I.checked}),k.addEventListener("change",()=>{j.audioMutedByDefault=k.checked}),S.addEventListener("input",()=>{j.defaultAudioVolume=Number(S.value)/100,ge()});for(const[C,Y]of F.entries())Y.addEventListener("change",()=>{if(Array.from(F.entries()).filter(([,z])=>z.checked).map(([z])=>z).length===0&&!j.lockedScaleId){Y.checked=!0;return}j.hiddenScaleIds=Array.from(F.keys()).filter(z=>{var Ve;return!((Ve=F.get(z))!=null&&Ve.checked)&&z!==j.lockedScaleId}),ke()}),C===j.lockedScaleId&&(Y.disabled=!0);we(e.initialView??"parameters"),ke();function we(C){l.dataset.view=C,C==="parameters"?(d.textContent=e.simClass.label,m.textContent="Shape Your Simulation",h.textContent=e.simClass.parameterSubtitle??"Adjust the parameters, inspect the setup, and press 'Run' when you're ready.",r.src=e.simClass.placeholderImage,r.alt=`${e.simClass.label} preview`):C==="settings"?(d.textContent="Interface",m.textContent="Adjust The Control Room",h.textContent="Change the interface theme and manage exhibit-level options for this installation.",r.src=Q("assets/Cluster_Stuart.webp"),r.alt="Galaxy cluster simulation preview"):(d.textContent="References",m.textContent="Project Sources And Attribution",h.textContent="Review the datasets, imagery, and supporting materials behind this experience.",r.src=Q("assets/synthetic_hst_pretty_galaxy.webp"),r.alt="Synthetic galaxy image preview"),C==="settings"?se.textContent="Apply":C==="credits"?se.textContent="Close":se.textContent="Run Simulation"}function ke(){ue.value=j.lockedScaleId??"",re.checked=j.manifestSource==="local",de.checked=j.manifestSource==="online",I.checked=j.verboseLogging,k.checked=j.audioMutedByDefault,S.value=String(Math.round(j.defaultAudioVolume*100)),ge();for(const[C,Y]of F.entries()){const J=j.lockedScaleId===C;Y.checked=J||!j.hiddenScaleIds.includes(C),Y.disabled=J}}function He(){if(R.value!==Ro){W.textContent="Incorrect password";return}R.value="",W.textContent="",Ye("open")}function Ye(C){Ie=C,$.dataset.state=C,x.textContent=C==="open"?"Hide Advanced Settings":"Advanced Settings",C!=="auth"&&(W.textContent="")}function B(){R.value="",W.textContent="",Ye("closed")}function q(){j=At(e.advancedSettings),ke()}function ge(){M.textContent=`${Math.round(Number(S.value))}%`}return se.addEventListener("click",()=>{const C=l.dataset.view;if(C==="settings"){e.onApplySettings(At(j));return}if(C==="credits"){e.onClose();return}e.onRun()}),{show(){t.hidden=!1,t.classList.remove("is-hidden")},hide(){t.hidden=!0,t.classList.add("is-hidden"),q(),B()},setSimulation(C,Y){e.simClass=C,a.dataset.simClass=C.id,pe.setSimClass(C,Y),l.dataset.view==="parameters"&&(r.src=C.placeholderImage,r.alt=`${C.label} preview`,we("parameters"))},setTheme(C){Ce.setActive(C)},setView(C){we(C),C!=="settings"&&B()},setAdvancedSettings(C){e.advancedSettings=At(C),j=At(C),ke(),B()}}}function At(n){return{lockedScaleId:n.lockedScaleId,manifestSource:n.manifestSource,verboseLogging:n.verboseLogging,hiddenScaleIds:[...n.hiddenScaleIds],audioMutedByDefault:n.audioMutedByDefault,defaultAudioVolume:n.defaultAudioVolume}}function Ho(n){const{TYPING_MS_PER_CHAR:e,MIN_TERMINAL_TIME_MS:t,FINAL_PAUSE_MS:s}=Jn,i=document.createElement("section");i.className="overlay overlay--initializing",i.hidden=!0,i.classList.add("is-hidden");const a=document.createElement("div");a.className="terminal";const r=document.createElement("div");r.className="terminal__header",r.innerHTML=`
    <div class="terminal__header-left">
      <span class="terminal__dot"></span>
      <span class="terminal__header-label">UNIVERSE_ENGINE_v9.5.1</span>
    </div>
  `;const o=document.createElement("div");o.className="terminal__log",a.appendChild(r),a.appendChild(o),i.appendChild(a),n.appendChild(i);let l=[],c=0;function u(){for(const b of l)window.clearTimeout(b);l=[]}function d(b,p){return new Promise(g=>{const _=window.setTimeout(()=>{p===c&&g()},Math.max(0,b));l.push(_)})}async function m(b,p){const g=document.createElement("div");g.className="terminal__line";const _=h();g.appendChild(_),o.appendChild(g);for(let v=0;v<b.length;v+=1){if(p!==c)return;const N=b[v];g.insertBefore(document.createTextNode(N),_),o.scrollTop=o.scrollHeight,await d(e,p)}_.remove()}function h(){const b=document.createElement("span");return b.className="terminal__cursor",b.textContent="█",b}return{async show(b,p,g,_){u(),c+=1;const v=c;i.hidden=!1,i.classList.remove("is-hidden");const N=performance.now(),E=(_==null?void 0:_.minTerminalTimeMs)??t;let k=!g,y=[...b];g&&g.then(()=>{k=!0});let L=0;for(;v===c;){y.length===0&&(y=[...b]);const M=Math.floor(Math.random()*y.length),[$]=y.splice(M,1),x=`${Hs(L)} ${$.text}`;if(L+=1,await m(x,v),v!==c)return;if(performance.now()-N>=E&&k)break}if(v!==c)return;const S=document.createElement("div");S.className="terminal__line terminal__line--syncing",S.textContent=`${Hs(L)} STARTING SIMULATION...`,o.appendChild(S),o.scrollTop=o.scrollHeight,await d(s,v),v===c&&p()},hide(){u(),c+=1,i.hidden=!0,i.classList.add("is-hidden"),o.innerHTML=""}}}function Hs(n){const e=Math.max(0,Math.floor(n)),t=Math.floor(e/3600),s=Math.floor(e%3600/60),i=e%60;return`[${Bn(t)}:${Bn(s)}:${Bn(i)}]`}function Bn(n){return String(n).padStart(2,"0")}function jo(n,e){const t=document.createElement("button");t.className="display-button",t.type="button",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Open configuration overlay"),n.appendChild(t);const s=document.createElement("div");s.className="display-menu";const i=document.createElement("div");i.className="display-menu__header",i.textContent="Menu",s.appendChild(i);const a=l("Home",()=>{c(),e.onHome()});s.appendChild(a),s.appendChild(l("Settings",()=>{c(),e.onViewSelected("settings")})),s.appendChild(l("Credits",()=>{c(),e.onViewSelected("credits")}));const r=l("Fullscreen",()=>{var d;c(),document.fullscreenElement?document.exitFullscreen():(d=document.getElementById("app"))==null||d.requestFullscreen()});s.appendChild(r),n.appendChild(s);function o(){const d=r.querySelector(".display-menu__item-label");d&&(d.textContent=document.fullscreenElement?"Exit Fullscreen":"Fullscreen");const m=document.getElementById("app");m&&m.classList.toggle("is-fullscreen",!!document.fullscreenElement)}return document.addEventListener("fullscreenchange",o),t.addEventListener("click",()=>{n.classList.toggle("open")}),document.addEventListener("click",d=>{n.contains(d.target)||c()}),u(e.showHome??!0),{close:c,setHomeVisible:u};function l(d,m){const h=document.createElement("button");return h.className="display-menu__item",h.type="button",h.innerHTML=`
...
`,Qo={planetary:Go,galaxy:zo,cosmos:Jo};function Xo(n){return We(Qo[n.id]).map(t=>({text:t}))}function Ft(n,e,t,s){if(e.length===0)return 0;const i=s?e.filter(r=>s.has(r.id)):e;return i.length===0?0:i.reduce((r,o)=>{var d;const l=t[o.id]??o.fallbackValue,c=((d=n.parameters)==null?void 0:d[o.id])??o.fallbackValue,u=Math.max(o.max-o.min,1e-9);return r+Math.abs(l-c)/u},0)/i.length}function Zo(n,e,t){if(n.length===0)return null;const s=new Set(e.filter(u=>u.primary!==!1).map(u=>u.id)),i=new Set(e.filter(u=>u.primary===!1).map(u=>u.id));if(!e.some(u=>u.primary===!1))return Un(n,e,t);const r=Un(n,e,t,s);if(!r)return null;const o=Ft(r,e,t,s),l=1e-6,c=n.filter(u=>{const d=Ft(u,e,t,s);return Math.abs(d-o)<=l});return Un(c,e,t,i)}function Un(n,e,t,s){if(n.length===0)return null;let i=n[0],a=Ft(i,e,t,s);for(const r of n.slice(1)){const o=Ft(r,e,t,s);o<a&&(i=r,a=o)}return i}function el(n){return n.replace(/\.mp4($|\?)/,".yaml$1")}async function tl(n){try{const e=await ze(n);if(!e.ok)return null;const t=await e.text(),s=We(t),i=ut(s.wallclockSeconds),a=ut(s.computeUsed),r=ut(s.memoryUsed),o=ut(s.carbonBurnt),l=ut(s.particlesUpdated),c=await nl(n),u=il(s.summaryMetrics);return i===null||a===null||r===null||o===null||l===null?null:{wallclockSeconds:i,computeUsed:a,memoryUsed:r,carbonBurnt:o,particlesUpdated:l,parameterValues:c,summaryMetrics:u}}catch{return null}}async function nl(n){try{const e=await ze(sl(n));if(!e.ok)return{};const t=await e.text(),s=We(t);return al(s)}catch{return{}}}function sl(n){return n.replace(/run_summary\.yaml($|\?)/,"parameters.yaml$1")}function ut(n){const e=typeof n=="number"?n:Number(n);return Number.isFinite(e)?e:null}function il(n){if(!n||typeof n!="object")return{};const e=n,t={};for(const[s,i]of Object.entries(e)){if(!i||typeof i!="object")continue;const a=i,r=typeof a.label=="string"?a.label:s,o=a.value;o!=null&&(t[s]={label:r,value:String(o)})}return t}function al(n){if(!n||typeof n!="object")return{};const e={};for(const[t,s]of Object.entries(n)){const i=ut(s);i!==null&&(e[t]=i)}return e}const ji="[UniverseEngine]";let qi=!1;function js(n){qi=n}function Ki(){return qi}function ce(n,e){Ki()&&console.info(ji,n,e??"")}function qe(n,e){Ki()&&console.warn(ji,n,e??"")}const rl={local:"assets/local-manifest.json",online:xi};function ol(n="local"){let e=n;const t=new Map,s=new Map;return{getSource(){return e},setSource(i){t.delete(i),s.delete(i),qr(),e=i,ce("Manifest source updated",{source:i})},async preloadActiveManifest(){await Yi(e,t,s)},async findNearestVideo(i,a,r){const o=await ul(e,t,s,i,a,r);if(o)return o;const l=Wi(i);return qe("Falling back to placeholder assets",{simClassId:i,source:e,fallbackUrl:l}),{url:l,liveDataUrl:ll(i),summaryUrl:el(l)}}}}function Wi(n){switch(n){case"planetary":return Q("assets/planet_test.mp4");case"galaxy":return Q("assets/galaxy_test.mp4");case"cosmos":return Q("assets/cosmo_test.mp4");default:return Q("assets/galaxy_test.mp4")}}function ll(n){switch(n){case"planetary":return Q("assets/planet_test_planetary_stats.csv");case"galaxy":return Q("assets/galaxy_test_galaxy_stats.csv");case"cosmos":return Q("assets/cosmo_test_cosmos_stats.csv");default:return Q("assets/galaxy_test_galaxy_stats.csv")}}async function Yi(n,e,t){const s=e.get(n);if(s)return s;const i=rl[n],a=(n==="online"?cl(i):fetch(Q(i),{cache:"no-store"}).then(async r=>{if(!r.ok)throw new Error(`Failed to load manifest: ${i}`);return ce("Loaded manifest",{source:n,manifestPath:i}),await r.json()})).then(r=>(t.set(n,dl()),r)).catch(r=>(t.delete(n),qe("Manifest unavailable",{source:n,manifestPath:i,error:r instanceof Error?r.message:String(r)}),{version:1,runs:[]}));return e.set(n,a),a}async function cl(n){const e=[n,Fn];for(const t of e)try{const s=await fetch(t,{cache:"no-store"});if(!s.ok)continue;const i=await s.json(),a=i.primaryBase??qs(xi),r=i.backupBase??qs(Fn);return jr(a,r),t===Fn&&Kr("backup"),ce("Loaded manifest",{source:"online",manifestPath:t,primaryBase:a,backupBase:r}),i}catch{continue}throw new Error(`Failed to load manifest: ${n}`)}async function ul(n,e,t,s,i,a){const o=(await Yi(n,e,t)).runs.filter(m=>m.simulationId===s);if(o.length===0)return qe("No manifest runs found for simulation",{simClassId:s,source:n}),null;const l=Zo(o,i,a);if(!l)return null;const c=Ft(l,i,a),u=l.defaultView??Object.keys(l.views)[0],d=l.views[u];return d?(ce("Selected manifest-backed run",{simClassId:s,source:n,runId:l.runId,selectedValues:a,distance:c,viewId:u}),{url:Qt(n,d,t),liveDataUrl:Qt(n,l.liveDataPath,t),summaryUrl:Qt(n,l.summaryPath,t),viewId:u,runId:l.runId,views:Object.fromEntries(Object.entries(l.views).map(([m,h])=>[m,Qt(n,h,t)]))}):null}function Qt(n,e,t){const s=n==="local"?Q(e):Ue(e),i=t.get(n);return i?Ai(s,"_manifest",i):s}function dl(){return`${Date.now()}`}function qs(n){const e=new URL(n);return`${e.protocol}//${e.host}`}const tn={mode:"time",frames:[]};async function fl(n){const e=await ze(n);if(!e.ok)throw new Error(`Failed to load live stats CSV: ${n}`);const t=await e.text();return pl(t)}function hl(n,e,t=0){if(n.mode==="row")return gl(n.frames,e,t);const s=n.frames;if(s.length===0)return{};if(e<=s[0].t)return{...s[0].values};const i=s[s.length-1];if(e>=i.t)return{...i.values};const a=ml(s,e),r=s[Math.max(0,a-1)],o=s[Math.min(s.length-1,a)],l=(e-r.t)/Math.max(o.t-r.t,1e-9);return yl(r.values,o.values,l)}function ml(n,e){let t=1,s=n.length-1;for(;t<s;){const i=Math.floor((t+s)/2);n[i].t<=e?t=i+1:s=i}return t}function pl(n){const e=n.split(/\r?\n/).map(s=>s.trim()).filter(Boolean);if(e.length<2)return tn;const t=Dn(e[0]);return t[0]==="t"?{mode:"time",frames:e.slice(1).map(s=>{const i=Dn(s),a={};for(let r=1;r<t.length;r+=1)a[t[r]]=i[r]??"";return{t:parseFloat(i[0]??"0")||0,values:a}})}:{mode:"row",frames:e.slice(1).map((s,i)=>{const a=Dn(s),r={};for(let o=0;o<t.length;o+=1)r[t[o]]=a[o]??"";return{t:i,values:r}})}}function gl(n,e,t){if(n.length===0)return{};if(!Number.isFinite(t)||t<=0)return{...n[0].values};const s=Math.max(0,Math.min(1,e/t)),i=Math.round(s*(n.length-1));return{...n[i].values}}function Dn(n){const e=[];let t="",s=!1;for(let i=0;i<n.length;i+=1){const a=n[i];if(a==='"'){s=!s;continue}if(a===","&&!s){e.push(t),t="";continue}t+=a}return e.push(t),e}function yl(n,e,t){const s=new Set([...Object.keys(n),...Object.keys(e)]),i={};for(const a of s){const r=n[a]??"",o=e[a]??r,l=parseFloat(r),c=parseFloat(o);if(Number.isFinite(l)&&Number.isFinite(c)){const u=l+(c-l)*t;i[a]=bl(u);continue}i[a]=t<.5?r:o}return i}function bl(n){return n.toFixed(2).replace(new RegExp("\\.0+$|(?<=\\..*?)0+$","g"),"").replace(/\.$/,"")}function wl(n){vl(po,n)}function vl(n,e){if(navigator.sendBeacon){const t=new Blob([JSON.stringify(e)],{type:"application/json"});if(navigator.sendBeacon(n,t)){ce("Run selection tracking dispatched",{simulationId:e.simulationId});return}}fetch(n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e),keepalive:!0}).then(t=>{t.ok?ce("Run selection tracked",{simulationId:e.simulationId}):qe("Run selection tracking rejected",{simulationId:e.simulationId,status:t.status})}).catch(t=>{qe("Run selection tracking failed",{simulationId:e.simulationId,error:t instanceof Error?t.message:String(t)})})}const Ks=50*1024*1024,_l=8,Sl=6e3,kl=8e3,El=5e3,Ll=1200,Nl=100,Hn={galaxy:"tron",planetary:"matrix",cosmos:"hal"};function Il(n){const e=ct.map(f=>f.id);let t=Fo(e),s=_s(t);const i=ol(t.manifestSource),a=Yo();js(t.verboseLogging),t.manifestSource==="online"&&i.preloadActiveManifest();let r=Ss(t.lockedScaleId)??s[0]??ct[0],o=t.lockedScaleId?Hn[r.id]:Ur(),l=!1,c=null,u=null,d=!1,m=t.audioMutedByDefault,h=t.defaultAudioVolume,b=0;const p=new Set;let g=null,_=0,v=tn,N=!1;const E=Object.fromEntries(ct.map(f=>[f.id,ra(f)]));Pn(o);const k=Wi(r.id),y=Yr(n,k),L=document.createElement("audio");L.preload="auto",L.hidden=!0,L.setAttribute("playsinline","true"),L.muted=m,L.volume=h,n.appendChild(L);const S=document.createElement("div");S.className="display-chrome",S.classList.add("is-hidden"),n.appendChild(S);const M=document.createElement("div");M.className="orientation-overlay",M.innerHTML=`
  ...
  `,n.appendChild(M);const $=document.createElement("div");$.className="swift-logo",$.innerHTML=`
    <img
    ...
    <img
      ...
      decoding="async"
    />
  `,n.appendChild($);const x=document.createElement("div");x.className="synth-logo is-hidden",x.innerHTML=`
    <img
    ...
    <img
      ...
      decoding="async"
    />
  `,n.appendChild(x);const U=document.createElement("img");U.className="app-partner-logo",U.src=Q("assets/dirac-hpc-white.webp"),U.alt="DIRAC HPC",U.decoding="async",n.appendChild(U);const R=document.createElement("div");R.className="display-chrome__top-left is-hidden",n.appendChild(R);const ie=jo(R,{onHome(){_n()},onViewSelected(f){if(f==="credits"){jt("credits");return}jt(f)},showHome:!t.lockedScaleId}),W=document.createElement("div");W.className="display-chrome__left-center",S.appendChild(W);const ne=Ao(W,{onSelect(f){ys(f)},onInfo(f,w,A){ue.textContent=w,_e.textContent=A,D.classList.add("is-visible")}}),D=document.createElement("div");D.className="view-info-overlay",D.innerHTML=`
  ...
  `,S.appendChild(de);const fe=Ko();y.setPlaybackRate(fe);const I=document.createElement("div");I.className="display-chrome__bottom",S.appendChild(I);const T=zr(I,{onChange(f){j(f)},onTogglePlay:gs,onAudioToggle:Zi,onSpeedChange:ea,onSummaryClick:Xi,onScrubStart(){He(),se()},onScrubEnd(){Ye(),y.isPaused()||Se()},initialSpeed:fe});T.setPlaying(!y.isPaused()),T.setAudioVisible(!1),T.setMuted(m),L.addEventListener("loadedmetadata",()=>{rt(!0),Ee()});let H=null,K=null,F=null,X=!1,V=null,oe=0;function Se(){if(H!==null)return;function f(){const w=y.getPlaybackFraction();T.setPosition(w),y.isPaused()?H=null:H=requestAnimationFrame(f)}H=requestAnimationFrame(f)}function se(){H!==null&&(cancelAnimationFrame(H),H=null)}function j(f){K=f,F===null&&(F=requestAnimationFrame(()=>{if(F=null,K===null)return;const w=K;K=null,y.seekToFraction(w),rt(!0)}))}function Ie(){if(K===null)return;F!==null&&(cancelAnimationFrame(F),F=null);const f=K;K=null,y.seekToFraction(f),rt(!0)}function pe(){V!==null&&(window.clearTimeout(V),V=null)}function Ce(){if(!(g!=null&&g.views))return[];const f=at(r,g);return Object.entries(g.views).filter(([w])=>w!==f).map(([,w])=>Ue(w)).filter(Boolean)}function we(){pe(),y.suspendPrewarming()}function ke(f=Ll){pe(),!(X||y.isPaused())&&(V=window.setTimeout(()=>{V=null,!(X||y.isPaused())&&(y.resumePrewarming(),y.prewarmSources(Ce()))},Math.max(0,f)))}function He(){X=!0,oe=0,we(),Ee()}function Ye(){X=!1,oe=0,Ie(),_=y.getPlaybackFraction()*y.getDurationSeconds(),Xe(_),ke(),Ee()}y.onPlayStateChange(f=>{T.setPlaying(!f),f?(se(),we()):(Se(),ke(0)),Ee()}),y.onTimeUpdate(f=>{if(_=f*y.getDurationSeconds(),X){const w=performance.now();if(w-oe<Nl)return;oe=w}Xe(_),rt()});const B=document.createElement("div");B.className="overlay-layer",n.appendChild(B);const q=No(B,{onReplay:Qi,onParameters:()=>jt("parameters"),onHome:_n,showHome:!t.lockedScaleId});y.onEnded(()=>{l=!0;const f=y.captureFrame();q.update(r,Ae(),y.getDurationSeconds(),c,f),q.show(),Ee()});const ge=lo(B,s,f=>{ps(f),jt("parameters")}),C=Do(B,{simClass:r,values:Ae(),theme:o,advancedSettings:t,availableScales:ct,onValuesChange:Gi,onThemeChange:vn,onRun:()=>{ce("Parameters submitted — starting run",{simClassId:r.id}),ta().catch(f=>{qe("Run failed to start",{simClassId:r.id,error:f instanceof Error?f.message:String(f)})})},onApplySettings:zi,onClose:Ji,initialView:"parameters"}),Y=Ho(B);T.setPosition(0),Xe(),q.hide();const J=new WeakMap,z=f=>{const w=J.get(f);w&&(clearTimeout(w),J.delete(f)),f.classList.remove("side-collapsed")},Ve=f=>{const w=J.get(f);w&&clearTimeout(w),J.set(f,setTimeout(()=>{f.classList.add("side-collapsed"),J.delete(f)},2500))},bn=f=>{const w=J.get(f);w&&(clearTimeout(w),J.delete(f)),f.classList.add("side-collapsed")},wn=(f,w)=>{const A=w.isCollapsible??(()=>!0);f.addEventListener("mouseenter",()=>z(f)),f.addEventListener("mouseleave",()=>{if(!A()){z(f);return}Ve(f)}),f.addEventListener("focusin",()=>z(f)),f.addEventListener("focusout",O=>{if(!f.contains(O.relatedTarget)){if(!A()){z(f);return}Ve(f)}}),f.addEventListener("click",()=>{if(!A()){z(f);return}if(f.classList.contains("side-collapsed")){z(f),Ve(f);return}w.toggleOnClick?bn(f):Ve(f)}),A()?bn(f):z(f)};wn(R,{toggleOnClick:!0,isCollapsible:()=>n.dataset.mode!=="entry"}),wn(W,{toggleOnClick:!0}),wn(I,{toggleOnClick:!1});let kt=0,it=null,Ht=0;const hs=()=>{it!==null&&(cancelAnimationFrame(it),it=null)},ms=()=>{if(it!==null)return;Ht=y.getPlaybackFraction();const f=()=>{if(kt===0){hs();return}const A=12*(1/60)/Math.max(y.getDurationSeconds(),1);Ht=Math.max(0,Math.min(1,Ht+kt*A)),y.seekToFraction(Ht),it=requestAnimationFrame(f)};it=requestAnimationFrame(f)};document.addEventListener("keydown",f=>{if(n.dataset.mode==="display"&&!(f.target instanceof HTMLInputElement||f.target instanceof HTMLTextAreaElement))switch(f.key){case"Escape":f.preventDefault(),D.classList.contains("is-visible")?D.classList.remove("is-visible"):_n();break;case" ":f.preventDefault(),gs();break;case"ArrowLeft":f.preventDefault(),z(I),Ve(I),kt=-1,ms();break;case"ArrowRight":f.preventDefault(),z(I),Ve(I),kt=1,ms();break;case"ArrowUp":case"ArrowDown":{if(f.preventDefault(),z(W),Ve(W),!(g!=null&&g.views)||Object.keys(g.views).length<=1)break;const w=r.views.filter(he=>{var ve;return((ve=g==null?void 0:g.views)==null?void 0:ve[he.id])!==void 0});if(w.length<=1)break;const A=g.viewId??at(r,g),O=w.findIndex(he=>he.id===A),le=f.key==="ArrowUp"?(O-1+w.length)%w.length:(O+1)%w.length;ys(w[le].id);break}}}),document.addEventListener("keyup",f=>{(f.key==="ArrowLeft"||f.key==="ArrowRight")&&(kt=0,hs())}),y.hideMedia(),y.pause(),Qe(t.lockedScaleId?"config":"entry");function ps(f){f.id===r.id&&N||(r=f,En(),vn(Hn[f.id]),C.setSimulation(r,Ae()),T.setPosition(0),Xe(),kn(),Sn())}function Gi(f){E[r.id]={...f},ce("Parameter values updated",{simClassId:r.id,values:E[r.id]}),Xe()}function vn(f){o=f,Pn(f),C.setTheme(f)}function jt(f){f==="parameters"&&C.setSimulation(r,Ae()),C.setView(f),Qe("config")}function zi(f){if(ba(f),N){q.hide(),Qe("display");return}C.setSimulation(r,Ae()),C.setView("parameters")}function Ji(){if(q.hide(),!N&&t.lockedScaleId){C.setSimulation(r,Ae()),C.setView("parameters");return}Qe(N?"display":"entry")}function _n(){t.lockedScaleId||(ce("Returning to home screen",{simClassId:r.id}),En(),N=!1,y.hideMedia(),Qe("entry"))}function Qi(){l=!1,q.hide(),y.getPlaybackFraction()>=.999&&(y.resetPlayback(),rt(!0)),Vn(y),Ee()}function Xi(){l=!0,y.pause();const f=c?y.captureFrame():null;q.update(r,Ae(),y.getDurationSeconds(),c,f),q.show(),Ee()}function gs(){y.isPaused()?Vn(y):y.pause()}function Zi(){m=!m,Ee()}function ea(f){y.setPlaybackRate(f),Wo(f),T.setSpeed(f)}async function ta(){const f=Ae(),w=a.start();ce("Run requested",{simClassId:r.id,values:f,manifestSource:i.getSource()});const A=await i.findNearestVideo(r.id,r.parameters,f);if(!a.isCurrent(w))return;En({preserveRunRequest:!0}),g=A;const O=at(r,A),le=Wr(i.getSource());wl({simulationId:r.id,parameters:f,manifestSource:i.getSource(),matchedRunId:A.runId,assetHostMode:le.mode,assetHostBase:le.base});const he=da(A,O)??A.url,ve=Object.entries(A.views??{}).filter(([xe])=>xe!==O).map(([,xe])=>xe);la(A.liveDataUrl,w),ca(A.summaryUrl,w),ha(A.summaryUrl,w),y.setMuted(!0),kn(O),Lt(),Qe("initializing");const qt=na(he);y.resumePrewarming(),y.prewarmSources(ve);const Nt=(async()=>{const xe=await qt;a.isCurrent(w)&&(ce(`Prepared active video source: ${xe.ownedObjectUrl?"FULL-FETCH":"PROGRESSIVE"}`,{selectedViewUrl:he,waitsForBuffer:xe.shouldWaitForBuffer}),y.setSource(xe.src,{ownedObjectUrl:xe.ownedObjectUrl}),y.pause(),await y.waitForLoadedData(kl),a.isCurrent(w)&&xe.shouldWaitForBuffer&&await y.waitForBufferedAhead(_l,Sl))})();await new Promise(xe=>{Y.show(Xo(r),xe,Nt,{minTerminalTimeMs:ga()})}),a.isCurrent(w)&&(N=!0,y.showMedia(),Vn(y),Qe("display"),Ee())}async function na(f){const w=Ue(f),A=await sa(f);if(A!==null&&A>0&&A<=Ks){ce("Downloading active video behind loading overlay",{videoUrl:w,contentLength:A});try{const O=await ze(f);if(!O.ok)throw new Error(`Failed to download active video: ${w}`);const le=await O.blob();return ce(`Active video full fetch complete: ${le.size} bytes`,{videoUrl:Ue(f),blobType:le.type}),{src:URL.createObjectURL(le),ownedObjectUrl:!0,shouldWaitForBuffer:!1}}catch(O){qe(`Full-fetch FAILED; falling back to progressive: ${O instanceof Error?O.message:String(O)}`,{videoUrl:f})}}return A!==null?ce("Active video exceeds full-fetch threshold; using progressive load",{videoUrl:f,contentLength:A,fullFetchMaxBytes:Ks}):ce("Could not determine active video size; using progressive load",{videoUrl:f}),ce("Using progressive active video load",{videoUrl:f}),{src:Ue(f),ownedObjectUrl:!1,shouldWaitForBuffer:!0}}async function sa(f){try{const w=await ze(f,{headers:{Range:"bytes=0-0"}});ce("Probed active video size with range request",{videoUrl:f,ok:w.ok,status:w.status,contentLength:w.headers.get("Content-Length"),contentRange:w.headers.get("Content-Range")});const A=aa(w.headers.get("Content-Length"));if(A!==null)return A;const O=ia(w.headers.get("Content-Range"));return O!==null?O:null}catch(w){return qe("Could not probe active video size",{videoUrl:f,error:w instanceof Error?w.message:String(w)}),null}}function ia(f){if(!f)return null;const w=f.match(/bytes\s+\d+-\d+\/(\d+)/i);if(!w)return null;const A=Number(w[1]);return Number.isFinite(A)&&A>0?A:null}function aa(f){if(!f)return null;const w=Number(f);return Number.isFinite(w)?w:null}function Qe(f){if(n.dataset.mode=f,f==="entry"?document.documentElement.setAttribute("data-theme","glass"):f==="display"&&Pn(o),Et(S,f==="display"||f==="config"),Et($,f==="display"||f==="entry"),Et(R,!t.lockedScaleId&&(f==="entry"||f==="config"||f==="display")),f==="entry"?z(R):bn(R),f==="entry"&&!t.lockedScaleId?ge.show():ge.hide(),f==="config"?(Y.hide(),C.setSimulation(r,Ae()),C.show()):C.hide(),f!=="display")q.hide();else if(l){const A=y.captureFrame();q.update(r,Ae(),y.getDurationSeconds(),c,A),q.show()}!N||f==="initializing"?(y.hideMedia(),f==="initializing"&&y.pause()):y.showMedia(),f!=="initializing"&&Y.hide(),Sn(),Ee()}function Sn(){if(n.dataset.mode==="entry"){Et(x,!0);return}const f=n.dataset.mode==="display",w=r.id==="galaxy";Et(x,f&&w)}function Xe(f=0){const w=hl(v,f,y.getDurationSeconds()),A=ua(r,c,f,y.getDurationSeconds());Pe.update(r,Ae(),{...w,...A})}function kn(f){const w=r.views.filter(le=>{var he;return((he=g==null?void 0:g.views)==null?void 0:he[le.id])!==void 0});if(w.length<=1){ne.hide(),ae.classList.add("is-hidden");return}const A=f??at(r,g),O=w.find(le=>le.id===A);ne.update(w,A),O?(ae.classList.remove("is-hidden"),ae.innerHTML=`<span class="viewport-title">${O.label??O.id}</span>`):ae.classList.add("is-hidden")}function En(f={}){f.preserveRunRequest||a.invalidate(),v=tn,l=!1,c=null,g=null,_=0,X=!1,K=null,pe(),F!==null&&(cancelAnimationFrame(F),F=null),q.hide(),ne.hide(),y.pause(),L.pause(),y.clearPrewarmedSources(),y.resetPlayback(),T.setPosition(0),vs()}function ys(f){if(!(g!=null&&g.views)||f===at(r,g))return;const w=Ue(g.views[f]);if(!w)return;g.viewId=f;const A=!y.isPaused()&&!l,O=l?0:y.getPlaybackFraction();l=!1,q.hide(),y.setSource(w,{seekFraction:O,autoplay:A}),y.prewarmSources(Ce()),A&&!X?ke():we(),kn(f),Lt(),Ee(),D.classList.remove("is-visible"),Sn()}function Ae(){return{...E[r.id]}}function ra(f){return Object.fromEntries(f.parameters.map(w=>[w.id,oa(w)]))}function oa(f){if(f.logScale){const he=Math.log10(f.min),ve=Math.log10(f.max);return 10**(he+Math.random()*(ve-he))}const w=Math.max(0,Math.round((f.max-f.min)/f.step)),A=Math.floor(Math.random()*(w+1)),O=f.min+A*f.step,le=Pi(f.step);return Number(O.toFixed(le))}async function la(f,w){let A=tn;try{A=await fl(f)}catch(O){qe("Failed to load live stats",{url:f,error:O instanceof Error?O.message:String(O)})}a.isCurrent(w)&&(v=A,Xe())}async function ca(f,w){const A=await tl(f);a.isCurrent(w)&&(c=A,Xe(_))}function ua(f,w,A,O){if(!w||!Number.isFinite(O)||O<=0)return{};const le=Math.max(0,Math.min(1,A/O)),he={};for(const ve of f.metadata.liveStats){if(!ve.live||!ve.fromVideo||!ve.scaleWithTime)continue;const qt=ve.videoKey??ve.id,Nt=w[qt];if(typeof Nt!="number"||!Number.isFinite(Nt))continue;const Ln=Nt*le;he[ve.id]=ve.integer?String(Math.floor(Ln)):String(Ln)}return he}function Et(f,w){f.hidden=!w,f.classList.toggle("is-hidden",!w)}function at(f,w){return w!=null&&w.views?w.viewId??Object.keys(w.views)[0]:w==null?void 0:w.viewId}function da(f,w){return!w||!f.views?null:f.views[w]??null}function bs(){const f=at(r,g);return f?r.views.some(w=>w.id===f&&w.audio):!1}function fa(f){return f.replace(/run_summary\.yaml($|\?)/,"audio_track.wav$1")}async function ha(f,w){const A=fa(f),O=Ue(A);if(p.has(O)){ws(O);return}const le=++b,he=await ma(A);if(!(!a.isCurrent(w)||le!==b)){if(!he){vs();return}p.add(O),ws(O)}}function ws(f){u=f,d=!0,L.src!==u&&(L.pause(),L.src=u,L.load()),Lt(),Ee()}async function ma(f){try{if((await ze(f,{method:"HEAD"})).ok)return!0}catch{}try{return(await ze(f,{headers:{Range:"bytes=0-0"}})).ok}catch{return!1}}function vs(){b+=1,u=null,d=!1,L.pause(),L.removeAttribute("src"),L.load(),Lt()}function pa(){m=t.audioMutedByDefault,h=t.defaultAudioVolume,L.muted=m,L.volume=h,T.setMuted(m)}function Lt(){T.setAudioVisible(bs()&&d&&!!u),T.setMuted(m)}function rt(f=!1){if(!d||!Number.isFinite(L.duration)||L.duration<=0)return;const w=Math.max(0,Math.min(L.duration,y.getPlaybackFraction()*L.duration));(f||Math.abs(L.currentTime-w)>.35)&&(L.currentTime=w)}function Ee(){const f=bs()&&d&&!!u;if(Lt(),L.muted=m,L.volume=h,!f){L.pause();return}if(rt(),n.dataset.mode!=="display"||y.isPaused()||l||X){L.pause();return}L.play().catch(()=>{m=!0,L.muted=!0,T.setMuted(!0)})}function _s(f){const w=new Set(Uo(f,e));return ct.filter(A=>w.has(A.id))}function Ss(f){return f?ct.find(w=>w.id===f)??null:null}function ga(){return i.getSource()!=="local"?Jn.MIN_TERMINAL_TIME_MS:ya(Jn.MIN_TERMINAL_TIME_MS,El)}function ya(f,w){const A=Math.ceil(Math.min(f,w)),O=Math.floor(Math.max(f,w));return Math.floor(Math.random()*(O-A+1))+A}function ba(f){const w=r.id,A=t.manifestSource;t=Bo(f,e),js(t.verboseLogging),s=_s(t),i.setSource(t.manifestSource),t.manifestSource==="online"&&i.preloadActiveManifest(),ie.setHomeVisible(!t.lockedScaleId),q.setHomeVisible(!t.lockedScaleId),ge.setSimulationClasses(s),C.setAdvancedSettings(t),ce("Advanced settings updated",t),pa(),Ee(),A!==t.manifestSource&&(g=null);const O=Ss(t.lockedScaleId);O&&(ps(O),O.id!==w&&(N=!1,y.hideMedia(),C.setView("parameters")),N||(vn(Hn[O.id]),C.setSimulation(r,Ae())))}}function Cl(){const n=document.getElementById("app");if(!n)throw new Error("App mount element not found.");Il(n)}Cl();
//# sourceMappingURL=main-DZxMjzt7.js.map
