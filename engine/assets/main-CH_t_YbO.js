...
`&&L!=="	"&&L!==" ")break}let h=t.substring(p);const y=h.indexOf(`
`);y===-1?f="-":t===h||y!==h.length-1?(f="+",a&&a()):f="",h&&(t=t.slice(0,-h.length),h[h.length-1]===`
`&&(h=h.slice(0,-1)),h=h.replace(is,`$&${c}`));let m=!1,g,S=-1;for(g=0;g<t.length;++g){const L=t[g];if(L===" ")m=!0;else if(L===`
`)S=g;else break}let v=t.substring(0,S<g?S+1:g);v&&(t=t.substring(v.length),v=v.replace(/\n+/g,`$&${c}`));let w=(m?c?"2":"1":"")+f;if(n&&(w+=" "+o(n.replace(/ ?[\r\n]+/g," ")),i&&i()),!u){const L=t.replace(/\n+/g,`
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${c}`);let I=!1;const N=En(s,!0);r!=="folded"&&e!==$.BLOCK_FOLDED&&(N.onOverflow=()=>{I=!0});const k=kn(`${v}${L}${h}`,c,ns,N);if(!I)return`>${w}
${c}${k}`}return t=t.replace(/\n+/g,`$&${c}`),`|${w}
${c}${v}${t}${h}`}function Ka(n,e,t,s){const{type:i,value:a}=n,{actualString:r,implicitKey:o,indent:l,indentStep:c,inFlow:u}=e;if(o&&a.includes(`
...
`+O.comment:P.comment=O.comment);const E=new Le(P,F);if(t.options.keepSourceTokens&&(E.srcToken=v),r){const A=c;Ri(t,A.items,P)&&i(k,"DUPLICATE_KEY","Map keys must be unique"),A.items.push(E)}else{const A=new Oe(t.schema);A.flow=!0,A.items.push(E);const W=(F??P).range;A.range=[P.range[0],W[1],W[2]],c.items.push(A)}f=F?F.range[2]:O.end}}const p=r?"}":"]",[h,...y]=s.end;let m=f;if((h==null?void 0:h.source)===p)m=h.offset+h.source.length;else{const S=o[0].toUpperCase()+o.substring(1),v=u?`${S} must end with a ${p}`:`${S} in block collection must be sufficiently indented and end with a ${p}`;i(f,u?"MISSING_CHAR":"BAD_INDENT",v),h&&h.source.length!==1&&y.unshift(h)}if(y.length>0){const S=Zt(y,m,t.options.strict,i);S.comment&&(c.comment?c.comment+=`
`+S.comment:c.comment=S.comment),c.range=[s.offset,m,S.offset]}else c.range=[s.offset,m,m];return c}function jn(n,e,t,s,i,a){const r=t.type==="block-map"?mr(n,e,t,s,a):t.type==="block-seq"?pr(n,e,t,s,a):gr(n,e,t,s,a),o=r.constructor;return i==="!"||i===o.tagName?(r.tag=o.tagName,r):(i&&(r.tag=i),r)}function yr(n,e,t,s,i){var p;const a=s.tag,r=a?e.directives.tagName(a.source,h=>i(a,"TAG_RESOLVE_FAILED",h)):null;if(t.type==="block-seq"){const{anchor:h,newlineAfterProp:y}=s,m=h&&a?h.offset>a.offset?h:a:h??a;m&&(!y||y.offset<m.offset)&&i(m,"MISSING_CHAR","Missing newline after block sequence props")}const o=t.type==="block-map"?"map":t.type==="block-seq"?"seq":t.start.source==="{"?"map":"seq";if(!a||!r||r==="!"||r===Oe.tagName&&o==="map"||r===ut.tagName&&o==="seq")return jn(n,e,t,i,r);let l=e.schema.tags.find(h=>h.tag===r&&h.collection===o);if(!l){const h=e.schema.knownTags[r];if((h==null?void 0:h.collection)===o)e.schema.tags.push(Object.assign({},h,{default:!1})),l=h;else return h?i(a,"BAD_COLLECTION_TYPE",`${h.tag} used for ${o} collection, but expects ${h.collection??"scalar"}`,!0):i(a,"TAG_RESOLVE_FAILED",`Unresolved tag: ${r}`,!0),jn(n,e,t,i,r)}const c=jn(n,e,t,i,r,l),u=((p=l.resolve)==null?void 0:p.call(l,c,h=>i(a,"TAG_RESOLVE_FAILED",h),e.options))??c,f=ue(u)?u:new $(u);return f.range=c.range,f.tag=r,l!=null&&l.format&&(f.format=l.format),f}function br(n,e,t){const s=e.offset,i=vr(e,n.options.strict,t);if(!i)return{value:"",type:null,comment:"",range:[s,s,s]};const a=i.mode===">"?$.BLOCK_FOLDED:$.BLOCK_LITERAL,r=e.source?wr(e.source):[];let o=r.length;for(let m=r.length-1;m>=0;--m){const g=r[m][1];if(g===""||g==="\r")o=m;else break}if(o===0){const m=i.chomp==="+"&&r.length>0?`
`.repeat(Math.max(1,r.length-1)):"";let g=s+i.length;return e.source&&(g+=e.source.length),{value:m,type:a,comment:i.comment,range:[s,g,g]}}let l=e.indent+i.indent,c=e.offset+i.length,u=0;for(let m=0;m<o;++m){const[g,S]=r[m];if(S===""||S==="\r")i.indent===0&&g.length>l&&(l=g.length);else{g.length<l&&t(c+g.length,"MISSING_CHAR","Block scalars with more-indented leading empty lines must use an explicit indentation indicator"),i.indent===0&&(l=g.length),u=m,l===0&&!n.atRoot&&t(c,"BAD_INDENT","Block scalar values in collections must be indented");break}c+=g.length+S.length+1}for(let m=r.length-1;m>=o;--m)r[m][0].length>l&&(o=m+1);let f="",p="",h=!1;for(let m=0;m<u;++m)f+=r[m][0].slice(l)+`
`;for(let m=u;m<o;++m){let[g,S]=r[m];c+=g.length+S.length+1;const v=S[S.length-1]==="\r";if(v&&(S=S.slice(0,-1)),S&&g.length<l){const w=`Block scalar lines must not be less indented than their ${i.indent?"explicit indentation indicator":"first line"}`;t(c-S.length-(v?2:1),"BAD_INDENT",w),g=""}a===$.BLOCK_LITERAL?(f+=p+g.slice(l)+S,p=`
`):g.length>l||S[0]==="	"?(p===" "?p=`
`:!h&&p===`
`&&(p=`
...
`?f+=`
`:p=`
`:(f+=p+S,p=" ",h=!1)}switch(i.chomp){case"-":break;case"+":for(let m=o;m<r.length;++m)f+=`
`+r[m][0].slice(l);f[f.length-1]!==`
`&&(f+=`
`);break;default:f+=`
`}const y=s+i.length+e.source.length;return{value:f,type:a,comment:i.comment,range:[s,y,y]}}function vr({offset:n,props:e},t,s){if(e[0].type!=="block-scalar-header")return s(e[0],"IMPOSSIBLE","Block scalar header not found"),null;const{source:i}=e[0],a=i[0];let r=0,o="",l=-1;for(let p=1;p<i.length;++p){const h=i[p];if(!o&&(h==="-"||h==="+"))o=h;else{const y=Number(h);!r&&y?r=y:l===-1&&(l=n+p)}}l!==-1&&s(l,"UNEXPECTED_TOKEN",`Block scalar header includes extra characters: ${i}`);let c=!1,u="",f=i.length;for(let p=1;p<e.length;++p){const h=e[p];switch(h.type){case"space":c=!0;case"newline":f+=h.source.length;break;case"comment":t&&!c&&s(h,"MISSING_CHAR","Comments must be separated from other tokens by white space characters"),f+=h.source.length,u=h.source.substring(1);break;case"error":s(h,"UNEXPECTED_TOKEN",h.message),f+=h.source.length;break;default:{const y=`Unexpected token in block scalar header: ${h.type}`;s(h,"UNEXPECTED_TOKEN",y);const m=h.source;m&&typeof m=="string"&&(f+=m.length)}}}return{mode:a,indent:r,chomp:o,comment:u,length:f}}function wr(n){const e=n.split(/\n( *)/),t=e[0],s=t.match(/^( *)/),a=[s!=null&&s[1]?[s[1],t.slice(s[1].length)]:["",t]];for(let r=1;r<e.length;r+=2)a.push([e[r],e[r+1]]);return a}function _r(n,e,t){const{offset:s,type:i,source:a,end:r}=n;let o,l;const c=(p,h,y)=>t(s+p,h,y);switch(i){case"scalar":o=$.PLAIN,l=Sr(a,c);break;case"single-quoted-scalar":o=$.QUOTE_SINGLE,l=kr(a,c);break;case"double-quoted-scalar":o=$.QUOTE_DOUBLE,l=Er(a,c);break;default:return t(n,"UNEXPECTED_TOKEN",`Expected a flow scalar value, but found: ${i}`),{value:"",type:null,comment:"",range:[s,s+a.length,s+a.length]}}const u=s+a.length,f=Zt(r,u,e,t);return{value:l,type:o,comment:f.comment,range:[s,u,f.offset]}}function Sr(n,e){let t="";switch(n[0]){case"	":t="a tab character";break;case",":t="flow indicator character ,";break;case"%":t="directive indicator character %";break;case"|":case">":{t=`block scalar indicator ${n[0]}`;break}case"@":case"`":{t=`reserved character ${n[0]}`;break}}return t&&e(0,"BAD_SCALAR_START",`Plain value cannot start with ${t}`),$i(n)}function kr(n,e){return(n[n.length-1]!=="'"||n.length===1)&&e(n.length,"MISSING_CHAR","Missing closing 'quote"),$i(n.slice(1,-1)).replace(/''/g,"'")}function $i(n){let e,t;try{e=new RegExp(`(.*?)(?<![ 	])[ 	]*\r?
`,"sy"),t=new RegExp(`[ 	]*(.*?)(?:(?<![ 	])[ 	]*)?\r?
...
${t.comment}`:t.comment}this.doc.range[2]=t.offset;break}default:this.errors.push(new jt(Bt(e),"UNEXPECTED_TOKEN",`Unsupported token ${e.type}`))}}*end(e=!1,t=-1){if(this.doc)this.decorate(this.doc,!0),yield this.doc,this.doc=null;else if(e){const s=Object.assign({_directives:this.directives},this.options),i=new Mn(void 0,s);this.atDirectives&&this.onError(t,"MISSING_CHAR","Missing directives-end indicator line"),i.range=[0,t,t],this.decorate(i,!1),yield i}}}const Ui="\uFEFF",Vi="",Di="",rs="";function Rr(n){switch(n){case Ui:return"byte-order-mark";case Vi:return"doc-mode";case Di:return"flow-error-end";case rs:return"scalar";case"---":return"doc-start";case"...":return"doc-end";case"":case`
`:case`\r
...
cosmos:
  ...
  simulationStats:
    - id: simulationVolume
      ...
      value: '--'
      description: The volume of this chunk of the universe.  When we run simulations we have to limit to a fixed volume of the universe that includes enough of what we want to study, while making sure the simulation doesn't become too costly.  We tend to describe this in terms of the length along a side, e.g. 25 Mpc/81.5M light years.
    - id: stellar_mass_msun
      ...
      display_format: compact
      precision: 3
      description: How much "star-stuff" your chunk of the universe managed to cook up across all of cosmic history.  In our Universe we would expect roughly 4.8 trillion Msun of stellar mass in this volume of the universe.
    - id: number_of_black_holes
      label: Black holes formed
      value: '--'
      display_format: integer
      description: How many supermassive black holes with masses greater than 50k Msun popped into existence over the course of your chunk of the universe's evolution.  In our Universe we would expect roughly 4,800 black holes in this volume of the universe.
    - id: particlesUpdated
      label: Particles updated
      value: '--'
      display_format: compact
      description: The sheer number of moving pieces the simulation had to keep track of every single frame.  In our Universe we would expect roughly 19.1 billion particle updates in this volume of the universe.
  similarityScore:
    value: '--'
...
cosmos:
  liveStats:
    ...
    - id: g_updates_total
      ...
      live: true
      integer: true
`;function ae(n){if(/^[a-z]+:\/\//i.test(n)||n.startsWith("data:")||n.startsWith("blob:"))return n;const e="/engine/",t=e.endsWith("/")?e:`${e}/`,s=n.startsWith("/")?n.slice(1):n;return`${t}${s}`}function ji(n,e,t){const s=n.indexOf("#"),i=s>=0?n.slice(0,s):n,a=s>=0?n.slice(s):"",r=new RegExp(`([?&])${Yr(e)}=[^&#]*`);if(r.test(i))return`${i.replace(r,`$1${e}=${encodeURIComponent(t)}`)}${a}`;const o=i.includes("?")?"&":"?";return`${i}${o}${e}=${encodeURIComponent(t)}${a}`}function Yr(n){return n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}const Gr=Xe(jr),zr=Xe(qr),Kn=Xe(Kr),Jr=Xe(Wr),bt=Object.entries(Gr).map(([n,e])=>{var r,o,l;const t=Qr(Kn[n]),s=(((r=Kn[n])==null?void 0:r.results)??[]).map(Xr),i=((o=Jr[n])==null?void 0:o.liveStats)??[],a=zr[n]??{};return{id:n,label:e.label,placeholderImage:ae(e.placeholderImage),parameterSubtitle:e.parameterSubtitle,metadata:{results:s,summaryStats:t.map(os),liveStats:i.map(os),morphologyChecklist:(l=Kn[n])==null?void 0:l.morphologyChecklist},parameters:Object.entries(a).map(([c,u])=>{const f=u.quali_labels,p=f!==void 0&&f.length>0,h=p?0:u.min,y=p?f.length-1:u.max,m=p?1:u.step??Zr(u.min,u.max),g=p?Math.floor(f.length/2):u.log_scale?Math.sqrt(u.min*u.max):eo(u.min,u.max);return{id:c,label:u.label,unit:u.unit??"",min:h,max:y,step:m,fallbackValue:g,description:u.description,valueScale:u.value_scale,displayUnit:u.display_unit,displayFormat:u.display_format,displaySignificantFigures:u.display_significant_figures,logScale:u.log_scale,qualiLabels:f,primary:u.primary??!0}}),views:(e.views??[]).map(c=>({id:c.id,label:c.label,icon:c.icon,audio:c.audio??!1,description:c.description}))}});function Qr(n){if(!n)return[];const e=[];for(const t of n.resources??[])e.push({...t,section:"resources"});for(const t of n.simulationStats??[])e.push({...t,section:"simulationStats"});return n.similarityScore&&e.push({id:"similarityScore",value:n.similarityScore.value}),e}function os(n){return{id:n.id,label:n.label,section:n.section,value:n.value,unit:n.unit,description:n.description,live:n.live??!1,liveKey:n.live_key,fromVideo:n.from_video??!1,videoKey:n.video_key,scaleWithTime:n.scale_with_time??!1,integer:n.integer??!1,valueScale:n.value_scale,displayFormat:n.display_format,precision:n.precision}}function Xr(n){return{...os(n),target:n.target}}function Zr(n,e){const s=Math.max(e-n,1e-9)/100,i=10**Math.floor(Math.log10(s)),a=s/i;let r=1;return a<=1?r=1:a<=2?r=2:a<=5?r=5:r=10,r*i}function eo(n,e){return n+(e-n)/2}const qi="universe-engine-theme",Ki=[{id:"glass",label:"Glass",icon:"[ ]"},{id:"matrix",label:"Matrix",icon:"[#]"},{id:"hal",label:"HAL 9000",icon:"( )"},{id:"nostromo",label:"Nostromo",icon:"[=]"},{id:"tron",label:"Tron",icon:"<>"}];function to(){const n=localStorage.getItem(qi);return so(n)?n:"glass"}function Wn(n){document.documentElement.setAttribute("data-theme",n),localStorage.setItem(qi,n)}function no(n,e,t){const s=document.createElement("div");s.className="theme-picker";const i=new Map;for(const r of Ki){const o=document.createElement("button");o.className="theme-picker__option",o.type="button",o.innerHTML=`
      <span class="theme-picker__icon">${r.icon}</span>
      <span class="theme-picker__label">${r.label}</span>
    `,o.addEventListener("click",()=>{a(r.id),t(r.id)}),s.appendChild(o),i.set(r.id,o)}n.appendChild(s),a(e);function a(r){for(const[o,l]of i.entries()){const c=o===r;l.classList.toggle("active",c),l.setAttribute("aria-pressed",String(c))}}return{setActive:a}}function so(n){return Ki.some(e=>e.id===n)}let ve=null,Ve="primary";function io(n,e=null){ve={primaryBase:n.replace(/\/+$/,""),backupBase:e?e.replace(/\/+$/,""):null}}function ao(){ve=null,Ve="primary"}function ro(n){Ve=n}function oo(n){return n==="local"?{mode:"local",base:null}:ve?{mode:Ve,base:Wi()}:{mode:"primary",base:null}}function Ke(n){if(!ve)return n;const e=Wi();if(!e)return n;if(/^https?:\/\//i.test(n)){const t=new URL(n);return!zs(t,ve.primaryBase)&&(!ve.backupBase||!zs(t,ve.backupBase))?n:Js(e,`${t.pathname}${t.search}${t.hash}`)}return Js(e,n)}async function tt(n,e){const t=Ke(n),s=!!(ve!=null&&ve.backupBase)&&Ve==="primary";try{const i=await fetch(t,e);if(i.ok||!s)return i;const a=await Gs(n,e);return a.ok&&(Ve="backup"),a}catch(i){if(!s)throw i;const a=await Gs(n,e);return a.ok&&(Ve="backup"),a}}function Wi(){return ve?Ve==="backup"&&ve.backupBase?ve.backupBase:ve.primaryBase:null}async function Gs(n,e){if(!(ve!=null&&ve.backupBase))throw new Error("Backup asset host is not configured.");const t=Ve;Ve="backup";try{const s=await fetch(Ke(n),e);return s.ok||(Ve=t),s}catch(s){throw Ve=t,s}}function zs(n,e){const t=new URL(e);return n.origin===t.origin}function Js(n,e){return`${n}${e.startsWith("/")?"":"/"}${e}`}function lo(n,e){const t=document.createElement("div");t.className="viewport";const s=document.createElement("video");s.className="viewport__media is-empty",s.crossOrigin="anonymous",s.src=e,s.loop=!1,s.muted=!0,s.playsInline=!0,s.preload="auto",s.setAttribute("aria-label","Simulation output"),t.appendChild(s),n.appendChild(t);let i,a,r,o=new Set,l=!1;const c=new Map,u=new Map,f=new Map;let p=null,h=null;const y=document.createElement("canvas"),m=y.getContext("2d");s.addEventListener("play",()=>r==null?void 0:r(!1)),s.addEventListener("pause",()=>r==null?void 0:r(!0)),s.addEventListener("ended",()=>r==null?void 0:r(!0)),s.addEventListener("timeupdate",()=>{!i||!Number.isFinite(s.duration)||s.duration<=0||i(s.currentTime/s.duration)}),s.addEventListener("ended",()=>{a==null||a()});let g=s.playbackRate;function S(){p&&(URL.revokeObjectURL(p),p=null)}function v(C,x={}){const D=u.get(C);D&&(u.delete(C),x={...x,ownedObjectUrl:!0},C=D),s.classList.add("fade-out"),window.setTimeout(()=>{if(s.src.endsWith(C)){s.classList.remove("fade-out");return}const K=s.muted,V=x.seekFraction;S(),h=null,p=x.ownedObjectUrl?C:null,s.src=C,s.load(),s.onloadeddata=()=>{if(s.muted=K,V!==void 0&&Number.isFinite(s.duration)&&s.duration>0){const _e=Math.max(0,Math.min(.999,V));s.currentTime=_e*s.duration}else s.currentTime=0;s.playbackRate=g,s.classList.remove("fade-out"),x.autoplay&&s.play().catch(()=>{})}},120)}function _(C){s.muted=C}async function w(){await s.play()}function L(){s.pause()}function I(){s.classList.add("is-empty")}function N(){s.removeAttribute("src"),s.load(),h=null}function k(){s.classList.remove("is-empty")}function P(C){if(!Number.isFinite(s.duration)||s.duration<=0)return;const x=Math.max(0,Math.min(1,C));s.currentTime=x*s.duration}function O(){s.currentTime=0,i==null||i(0)}function F(C=8e3){return s.readyState>=HTMLMediaElement.HAVE_CURRENT_DATA?Promise.resolve():new Promise(x=>{const D=()=>{V(),x()},K=window.setTimeout(()=>{V(),x()},Math.max(0,C));function V(){window.clearTimeout(K),s.removeEventListener("loadeddata",D)}s.addEventListener("loadeddata",D,{once:!0})})}function E(C,x=8e3){const D=Math.max(0,C);return D===0||W(D)?Promise.resolve():new Promise(K=>{const V=()=>{W(D)&&(te(),K())},_e=window.setTimeout(()=>{te(),K()},Math.max(0,x));function te(){window.clearTimeout(_e),s.removeEventListener("progress",V),s.removeEventListener("canplay",V),s.removeEventListener("loadeddata",V)}s.addEventListener("progress",V),s.addEventListener("canplay",V),s.addEventListener("loadeddata",V),V()})}function A(C=250){return!s.seeking&&s.readyState>=HTMLMediaElement.HAVE_CURRENT_DATA?Promise.resolve():new Promise(x=>{const D=()=>{s.seeking||s.readyState<HTMLMediaElement.HAVE_CURRENT_DATA||(V(),x())},K=window.setTimeout(()=>{V(),x()},Math.max(0,C));function V(){window.clearTimeout(K),s.removeEventListener("seeked",D),s.removeEventListener("canplay",D),s.removeEventListener("loadeddata",D)}s.addEventListener("seeked",D),s.addEventListener("canplay",D),s.addEventListener("loadeddata",D),D()})}function W(C){const x=s.currentTime;for(let D=0;D<s.buffered.length;D+=1){const K=s.buffered.start(D),V=s.buffered.end(D);if(!(x<K||x>V))return V-x>=C}return!1}function fe(C){o=new Set(C.filter(Boolean).filter(x=>x!==s.currentSrc)),l||Y()}function J(){l=!0,Z(),he()}function se(){if(!l){Y();return}l=!1,Y()}function Y(){for(const[C,x]of c.entries())o.has(C)||(x.removeAttribute("src"),x.load(),c.delete(C));for(const[C,x]of f.entries())o.has(C)||(x.abort(),f.delete(C));for(const C of o){if(!c.has(C)){const x=document.createElement("video");x.preload="auto",x.crossOrigin="anonymous",x.muted=!0,x.playsInline=!0,x.src=Ke(C),x.load(),c.set(C,x)}u.has(C)||f.has(C)||re(C)}}function Z(){for(const C of c.values())C.removeAttribute("src"),C.load();c.clear()}function he(){for(const C of f.values())C.abort();f.clear()}function re(C){const x=new AbortController;f.set(C,x);const D=ji(C,"_",`${Date.now()}`);tt(D,{signal:x.signal}).then(async K=>{if(!K.ok)return;const V=await K.blob();o.has(C)&&u.set(C,URL.createObjectURL(V))}).catch(K=>{K instanceof DOMException&&K.name}).finally(()=>{f.get(C)===x&&f.delete(C)})}function we(){o.clear(),l=!1,Z(),he();for(const C of u.values())URL.revokeObjectURL(C);u.clear()}function Q(C){return u.get(C)??null}function Ne(){if(!(!m||s.readyState<2||s.videoWidth===0||s.videoHeight===0)){y.width=s.videoWidth,y.height=s.videoHeight;try{m.drawImage(s,0,0,y.width,y.height),h=y.toDataURL("image/jpeg",.85)}catch{h=null}}}function me(){return Ne(),h}function Re(C){i=C}function ee(C){a=C}return{setSource:v,setMuted:_,play:w,pause:L,hideMedia:I,clearSource:N,showMedia:k,seekToFraction:P,resetPlayback:O,waitForLoadedData:F,waitForBufferedAhead:E,waitForSeekSettled:A,onTimeUpdate:Re,onEnded:ee,getDurationSeconds:()=>Number.isFinite(s.duration)?s.duration:0,getCurrentTimeSeconds:()=>Number.isFinite(s.currentTime)?s.currentTime:0,getPlaybackFraction:()=>!Number.isFinite(s.duration)||s.duration<=0?0:s.currentTime/s.duration,isPaused:()=>s.paused,setPlaybackRate:C=>{g=C,s.playbackRate=C},getPlaybackRate:()=>g,onPlayStateChange:C=>{r=C},getElement:()=>t,prewarmSources:fe,suspendPrewarming:J,resumePrewarming:se,clearPrewarmedSources:we,getPrewarmedBlobUrl:Q,captureFrame:me}}const co=[.25,.5,1,2];function uo(n,e={}){const{onChange:t,onTogglePlay:s,onSpeedChange:i,onSummaryClick:a,onAudioToggle:r,onScrubStart:o,onScrubEnd:l,initialSpeed:c=1}=e,u=document.createElement("div");let f=!1;u.className="timeline";const p=document.createElement("div");p.className="timeline__bar-row";const h=document.createElement("div");h.className="timeline__audio is-hidden";const y=document.createElement("button");y.className="timeline__audio-btn",y.type="button",y.setAttribute("aria-label","Toggle audio mute"),y.innerHTML=fo(),y.addEventListener("click",()=>r==null?void 0:r()),h.appendChild(y);const m=document.createElement("button");m.className="timeline__play-btn",m.type="button",m.setAttribute("aria-label","Toggle playback"),m.addEventListener("click",()=>s==null?void 0:s());const g=document.createElement("input");g.className="timeline__slider",g.type="range",g.min="0",g.max="1000",g.step="1",g.value="0",g.style.setProperty("--fill","0%"),g.setAttribute("aria-label","Simulation time");const S=document.createElement("div");S.className="timeline__speed";const v=document.createElement("button");v.className="timeline__speed-btn",v.type="button",v.setAttribute("aria-label","Playback speed"),v.addEventListener("click",()=>{S.classList.toggle("open")});const _=document.createElement("div");_.className="timeline__speed-menu";for(const k of co){const P=document.createElement("button");P.className="timeline__speed-option",P.type="button",P.textContent=Yn(k),P.addEventListener("click",()=>{S.classList.remove("open"),i==null||i(k)}),_.appendChild(P)}S.appendChild(v),S.appendChild(_);const w=document.createElement("button");w.className="timeline__summary-btn",w.type="button",w.setAttribute("aria-label","View run summary"),w.textContent="ⓘ",w.addEventListener("click",()=>a==null?void 0:a()),p.appendChild(h),p.appendChild(m),p.appendChild(g),p.appendChild(S),p.appendChild(w);let L=null,I=null;return g.addEventListener("input",()=>{const k=parseInt(g.value,10)/1e3;g.style.setProperty("--fill",`${k*100}%`),L=k,I===null&&(I=requestAnimationFrame(()=>{if(I=null,L===null)return;const P=L;L=null,t==null||t(P)}))}),g.addEventListener("pointerdown",()=>{f||(f=!0,o==null||o())}),g.addEventListener("pointerup",()=>{f&&(f=!1,l==null||l())}),g.addEventListener("change",()=>{f&&(f=!1,l==null||l())}),document.addEventListener("click",k=>{S.contains(k.target)||S.classList.remove("open")}),u.appendChild(p),n.appendChild(u),N(c),{setPosition(k){const P=Math.max(0,Math.min(1,k));g.value=String(Math.round(P*1e3)),g.style.setProperty("--fill",`${P*100}%`)},setPlaying(k){m.textContent=k?"❚❚︎":"▶︎",m.classList.toggle("is-paused",!k),m.setAttribute("aria-label",k?"Pause":"Play")},setSpeed(k){N(k)},setAudioVisible(k){h.hidden=!k,h.classList.toggle("is-hidden",!k)},setMuted(k){y.classList.toggle("is-muted",k),y.setAttribute("aria-label",k?"Unmute audio":"Mute audio")}};function N(k){v.textContent=Yn(k);for(const P of _.children)P.classList.toggle("is-active",P.textContent===Yn(k))}}function Yn(n){return`x${n}`}function fo(){return`
...
cosmos:
  stellar_fraction:
    greenLow: Very close.  Your universe locked just a slightly smaller fraction of its mass into stars than ours.  In our Universe roughly 0.79% of the mass ended up locked inside stars.
    greenHigh: Very close.  Your universe locked just a slightly larger fraction of its mass into stars than ours.  In our Universe roughly 0.79% of the mass ended up locked inside stars.
    amberLow: A bit low.  Your universe turned less of its raw material into stars, leaving it a bit darker and emptier.  In our Universe roughly 0.79% of the mass ended up locked inside stars.
    amberHigh: A bit high.  Your universe was a bit more efficient at forging gas into stars than our own.  In our Universe roughly 0.79% of the mass ended up locked inside stars.
    redLow: Far too low.  Your universe struggled to form stars — most of the mass is still drifting around as gas.  In our Universe roughly 0.79% of the mass ended up locked inside stars.
    redHigh: Far too high.  Your universe is bursting with stars — it converted far more mass into starlight than ours.  In our Universe roughly 0.79% of the mass ended up locked inside stars.
  average_temperature_kelvin:
    greenLow: Very close.  The gas in your universe is just a shade cooler than in ours.  In our Universe the average gas temperature is roughly 902,000 K.
    greenHigh: Very close.  The gas in your universe is just a shade warmer than in ours.  In our Universe the average gas temperature is roughly 902,000 K.
    amberLow: A bit cool.  Your gas is cooler than expected — less energetic feedback from black holes and supernovae heating things up.  In our Universe the average gas temperature is roughly 902,000 K.
    amberHigh: A bit hot.  Your gas is warmer than average — more energetic black holes and stars are heating up the cosmos.  In our Universe the average gas temperature is roughly 902,000 K.
    redLow: Far too cool.  The gas in your universe is very cold — there is barely any energetic feedback happening at all.  In our Universe the average gas temperature is roughly 902,000 K.
    redHigh: Far too hot.  The gas is scorching — black holes and stars are pumping so much energy that normal galaxy growth is disrupted.  In our Universe the average gas temperature is roughly 902,000 K.
  nhalos:
    greenLow: Very close.  Your universe built just a fraction fewer galaxy-sized dark matter halos than ours.  In our Universe roughly 1,600 galaxy halos form in this volume.
    greenHigh: Very close.  Your universe built just a fraction more galaxy-sized dark matter halos than ours.  In our Universe roughly 1,600 galaxy halos form in this volume.
    amberLow: A bit low.  Fewer galaxy-sized dark matter clumps formed — your universe needed a bit more time or stronger gravity to pull them together.  In our Universe roughly 1,600 galaxy halos form in this volume.
    amberHigh: A bit high.  More galaxy-sized dark matter clumps popped up than in ours — structure formation was a little more eager.  In our Universe roughly 1,600 galaxy halos form in this volume.
    redLow: Far too low.  Very few galaxy halos managed to form — structures barely clumped together.  In our Universe roughly 1,600 galaxy halos form in this volume.
    redHigh: Far too high.  Galaxy halos are everywhere — your universe is absolutely teeming with dark matter structures.  In our Universe roughly 1,600 galaxy halos form in this volume.
...
- text: Sonifications developed with STRAUSS (Trayford et al. 2025)
  url: https://strauss.readthedocs.io/en/latest/
...
`;function Yo(){const n=Xe(Wo);if(!Array.isArray(n))return[];const e=[];for(const t of n){if(!t||typeof t!="object")continue;const s=t.text;if(typeof s!="string"||s.trim().length===0)continue;const i={text:s},a=t.url;typeof a=="string"&&a.trim().length>0&&(i.url=a);const r=t.logo;typeof r=="string"&&r.trim().length>0&&(i.logo=r),t.header===!0&&(i.header=!0),e.push(i)}return e}function Go(n,e,t,s){const i=document.createElement("div");i.className="parameter-editor",n.appendChild(i);let a=e,r={...t};function o(u,f){a=u,r=f?{...f}:zo(u),i.innerHTML="";const p=document.createElement("div");p.className="parameter-editor__heading",p.innerHTML=`
 ...
 Engine</h1>
  ...
  `;const p=f.querySelector(".config-overlay__eyebrow"),h=f.querySelector(".config-overlay__title"),y=f.querySelector(".config-overlay__subtitle"),m=document.createElement("button");m.className="config-overlay__close",m.type="button",m.setAttribute("aria-label","Back"),m.textContent="←",u.appendChild(f),u.appendChild(m);const g=document.createElement("section");g.className="config-overlay__section config-overlay__section--grow",g.dataset.section="parameters";const S=document.createElement("div");g.appendChild(S);const v=document.createElement("section");v.className="config-overlay__section config-overlay__section--grow",v.dataset.section="settings";const _=document.createElement("div");_.className="config-overlay__settings-block",_.innerHTML=`
  ...
  `,C.appendChild(x),C.appendChild(D),Z.appendChild(C);const K=document.createElement("label");K.className="advanced-settings__field advanced-settings__field--inline";const V=document.createElement("input"),_e=document.createElement("span");V.type="checkbox",V.className="advanced-settings__checkbox",_e.innerHTML=`
  ...
  `,K.appendChild(V),K.appendChild(_e),Z.appendChild(K);const te=document.createElement("div");te.className="advanced-settings__field",te.innerHTML=`
  ...
  `;const X=oe.querySelector("[data-credits]"),ke=Yo();if(X.innerHTML="",ke.length===0){const M=document.createElement("div");M.className="credits-list__entry",M.textContent="To be credited...",X.appendChild(M)}else for(const M of ke)if(M.header){const B=document.createElement("div");B.className="credits-list__heading",B.textContent=M.text,X.appendChild(B)}else{const B=document.createElement("div");B.className="credits-list__entry";const ge=document.createElement("span");if(ge.className="credits-list__text",M.url){const q=document.createElement("a");q.className="credits-list__link",q.href=M.url,q.target="_blank",q.rel="noopener noreferrer",q.textContent=M.text,ge.appendChild(q)}else ge.textContent=M.text;B.appendChild(ge),X.appendChild(B)}const pe=document.createElement("div");pe.className="config-overlay__footer";const ie=document.createElement("button");ie.className="run-button",ie.type="button",ie.textContent="Run",pe.appendChild(ie),c.appendChild(u),c.appendChild(g),c.appendChild(v),c.appendChild(oe),c.appendChild(pe),i.appendChild(a),i.appendChild(c),s.appendChild(i),t.appendChild(s),n.appendChild(t);let U=Vt(e.advancedSettings),Ce="closed",je=!e.advancedSettings.lockedScaleId;const Ye=Go(S,e.simClass,e.values,e.onValuesChange),ft=no(_,e.theme,e.onThemeChange);m.addEventListener("click",e.onClose),W.addEventListener("click",()=>{if(Ce==="open"){$e("closed");return}$e("auth"),J.focus()}),se.addEventListener("click",Ee),J.addEventListener("keydown",M=>{M.key==="Enter"&&Ee()}),re.addEventListener("change",()=>{U.lockedScaleId=re.value||null,le()}),me.addEventListener("change",()=>{me.checked&&(U.manifestSource="local")}),ee.addEventListener("change",()=>{ee.checked&&(U.manifestSource="online")}),x.addEventListener("change",()=>{U.verboseLogging=x.checked}),k.addEventListener("change",()=>{U.audioMutedByDefault=k.checked}),F.addEventListener("input",()=>{U.defaultAudioVolume=Number(F.value)/100,en()}),V.addEventListener("change",()=>{U.lockFullscreen=V.checked});for(const[M,B]of G.entries())B.addEventListener("change",()=>{if(Array.from(G.entries()).filter(([,q])=>q.checked).map(([q])=>q).length===0&&!U.lockedScaleId){B.checked=!0;return}U.hiddenScaleIds=Array.from(G.keys()).filter(q=>{var ht;return!((ht=G.get(q))!=null&&ht.checked)&&q!==U.lockedScaleId}),le()}),M===U.lockedScaleId&&(B.disabled=!0);j(e.initialView??"parameters"),le(),Ge(je);function j(M){c.dataset.view=M,M==="parameters"?(p.textContent=e.simClass.label,h.textContent="Shape Your Simulation",y.textContent=e.simClass.parameterSubtitle??"Adjust the parameters, inspect the setup, and press 'Run' when you're ready.",o.textContent=e.simClass.label,o.hidden=!1,r.src=e.simClass.placeholderImage,r.alt=`${e.simClass.label} preview`):M==="settings"?(p.textContent="Interface",h.textContent="Adjust The Control Room",y.textContent="Change the interface theme and manage exhibit-level options for this installation.",o.textContent="",o.hidden=!0,r.src=ae("assets/Cluster_Stuart.webp"),r.alt="Galaxy cluster simulation preview"):(p.textContent="References",h.textContent="Project Sources And Attribution",y.textContent="Review the datasets, imagery, and supporting materials behind this experience.",o.textContent="",o.hidden=!0,r.src=ae("assets/synthetic_hst_pretty_galaxy.webp"),r.alt="Synthetic galaxy image preview"),M==="settings"?ie.textContent="Apply":ie.textContent="Run Simulation",pe.hidden=M==="credits",tn()}function le(){re.value=U.lockedScaleId??"",me.checked=U.manifestSource==="local",ee.checked=U.manifestSource==="online",x.checked=U.verboseLogging,k.checked=U.audioMutedByDefault,V.checked=U.lockFullscreen,F.value=String(Math.round(U.defaultAudioVolume*100)),en();for(const[M,B]of G.entries()){const ge=U.lockedScaleId===M;B.checked=ge||!U.hiddenScaleIds.includes(M),B.disabled=ge}}function Ee(){if(J.value!==Jo){Y.textContent="Incorrect password";return}J.value="",Y.textContent="",$e("open")}function $e(M){Ce=M,A.dataset.state=M,W.textContent=M==="open"?"Hide Advanced Settings":"Advanced Settings",M!=="auth"&&(Y.textContent="")}function qe(){J.value="",Y.textContent="",$e("closed")}function st(){U=Vt(e.advancedSettings),le()}function en(){E.textContent=`${Math.round(Number(F.value))}%`}function tn(){const M=c.dataset.view,B=M==="settings"||M==="credits"||je;m.hidden=!B,m.classList.toggle("is-hidden",!B),m.setAttribute("aria-label",M==="parameters"?"Back":"Close"),m.textContent=M==="parameters"?"←":"×"}function Ge(M){je=M,tn()}return ie.addEventListener("click",()=>{if(c.dataset.view==="settings"){e.onApplySettings(Vt(U));return}e.onRun()}),{show(){t.hidden=!1,t.classList.remove("is-hidden")},hide(){t.hidden=!0,t.classList.add("is-hidden"),st(),qe()},setSimulation(M,B){e.simClass=M,a.dataset.simClass=M.id,Ye.setSimClass(M,B),c.dataset.view==="parameters"&&(r.src=M.placeholderImage,r.alt=`${M.label} preview`,j("parameters"))},setTheme(M){ft.setActive(M)},setView(M){j(M),M!=="settings"&&qe()},setAdvancedSettings(M){e.advancedSettings=Vt(M),U=Vt(M),le(),qe()},setBackVisible:Ge}}function Vt(n){return{lockedScaleId:n.lockedScaleId,manifestSource:n.manifestSource,verboseLogging:n.verboseLogging,hiddenScaleIds:[...n.hiddenScaleIds],audioMutedByDefault:n.audioMutedByDefault,defaultAudioVolume:n.defaultAudioVolume,lockFullscreen:n.lockFullscreen}}function nl(n){const{TYPING_MS_PER_CHAR:e,MIN_TERMINAL_TIME_MS:t,FINAL_PAUSE_MS:s}=ls,i=document.createElement("section");i.className="overlay overlay--initializing",i.hidden=!0,i.classList.add("is-hidden");const a=document.createElement("div");a.className="terminal";const r=document.createElement("div");r.className="terminal__header",r.innerHTML=`
    <div class="terminal__header-left">
      <span class="terminal__dot"></span>
      <span class="terminal__header-label">UNIVERSE_ENGINE_v9.5.1</span>
    </div>
  `;const o=document.createElement("div");o.className="terminal__log",a.appendChild(r),a.appendChild(o),i.appendChild(a),n.appendChild(i);let l=[],c=0;function u(){for(const y of l)window.clearTimeout(y);l=[]}function f(y,m){return new Promise(g=>{const S=window.setTimeout(()=>{m===c&&g()},Math.max(0,y));l.push(S)})}async function p(y,m){const g=document.createElement("div");g.className="terminal__line";const S=h();g.appendChild(S),o.appendChild(g);for(let v=0;v<y.length;v+=1){if(m!==c)return;const _=y[v];g.insertBefore(document.createTextNode(_),S),o.scrollTop=o.scrollHeight,await f(e,m)}S.remove()}function h(){const y=document.createElement("span");return y.className="terminal__cursor",y.textContent="█",y}return{async show(y,m,g,S){u(),c+=1;const v=c;i.hidden=!1,i.classList.remove("is-hidden");const _=performance.now(),w=(S==null?void 0:S.minTerminalTimeMs)??t;let L=!g,I=[...y];g&&g.then(()=>{L=!0});let N=0;for(;v===c;){I.length===0&&(I=[...y]);const P=Math.floor(Math.random()*I.length),[O]=I.splice(P,1),F=`${ni(N)} ${O.text}`;if(N+=1,await p(F,v),v!==c)return;if(performance.now()-_>=w&&L)break}if(v!==c)return;const k=document.createElement("div");k.className="terminal__line terminal__line--syncing",k.textContent=`${ni(N)} STARTING SIMULATION...`,o.appendChild(k),o.scrollTop=o.scrollHeight,await f(s,v),v===c&&m()},hide(){u(),c+=1,i.hidden=!0,i.classList.add("is-hidden"),o.innerHTML=""}}}function ni(n){const e=Math.max(0,Math.floor(n)),t=Math.floor(e/3600),s=Math.floor(e%3600/60),i=e%60;return`[${Qn(t)}:${Qn(s)}:${Qn(i)}]`}function Qn(n){return String(n).padStart(2,"0")}function sl(n,e){const t=document.createElement("button");t.className="display-button",t.type="button",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Open configuration overlay"),n.appendChild(t);const s=document.createElement("div");s.className="display-menu";const i=document.createElement("div");i.className="display-menu__header",i.textContent="Menu",s.appendChild(i);const a=c("Home",()=>{f(),e.onHome()}),r=c("Select Parameters",()=>{f(),e.onParameters()});s.appendChild(a),s.appendChild(r),s.appendChild(c("Settings",()=>{f(),e.onViewSelected("settings")})),s.appendChild(c("Credits",()=>{f(),e.onViewSelected("credits")})),s.appendChild(u("More Info","https://universemakers.org/"));const o=c("Fullscreen",()=>{var h;f(),document.fullscreenElement?document.exitFullscreen():(h=document.getElementById("app"))==null||h.requestFullscreen()});o.classList.add("display-menu__fullscreen"),s.appendChild(o),n.appendChild(s);function l(){const h=o.querySelector(".display-menu__item-label");h&&(h.textContent=document.fullscreenElement?"Exit Fullscreen":"Fullscreen");const y=document.getElementById("app");y&&y.classList.toggle("is-fullscreen",!!document.fullscreenElement)}return document.addEventListener("fullscreenchange",l),t.addEventListener("click",()=>{n.classList.toggle("open")}),document.addEventListener("click",h=>{n.contains(h.target)||f()}),p(e.showHome??!0),{close:f,setHomeVisible:p,setFullscreenVisible(h){o.hidden=!h,o.classList.toggle("is-hidden",!h)}};function c(h,y){const m=document.createElement("button");return m.className="display-menu__item",m.type="button",m.innerHTML=`
      <span class="display-menu__item-mark"></span>
      <span class="display-menu__item-label">${h}</span>
    `,m.addEventListener("click",y),m}function u(h,y){const m=document.createElement("a");return m.className="display-menu__item",m.href=y,m.innerHTML=`
      <span class="display-menu__item-mark"></span>
      <span class="display-menu__item-label">${h}</span>
    `,m}function f(){n.classList.remove("open")}function p(h){a.hidden=!h,a.classList.toggle("is-hidden",!h),r.hidden=h,r.classList.toggle("is-hidden",h)}}const sa="universe-engine-playback-speed",il=new Set([.25,.5,1,2]);function al(){const n=localStorage.getItem(sa),e=n?Number(n):NaN;return il.has(e)?e:1}function rl(n){localStorage.setItem(sa,String(n))}async function hn(n){try{await n.play()}catch{n.setMuted(!0);try{await n.play()}catch{}}}function ol(){let n=0;return{start(){return n+=1,n},invalidate(){n+=1},isCurrent(e){return e===n}}}const ll=`# Initialization terminal script for the Planetary simulation family.
#
