...
`,eo={planetary:Qr,galaxy:Xr,cosmos:Zr};function to(n){return Me(eo[n.id]).map(t=>({text:t}))}function no(n){return n.replace(/\.mp4($|\?)/,".yaml$1")}async function so(n){try{const e=await fetch(n);if(!e.ok)return null;const t=await e.text(),s=Me(t),i=Qe(s.wallclockSeconds),a=Qe(s.computeUsed),r=Qe(s.memoryUsed),o=Qe(s.carbonBurnt),c=Qe(s.particlesUpdated),l=await io(n),u=ro(s.summaryMetrics);return i===null||a===null||r===null||o===null||c===null?null:{wallclockSeconds:i,computeUsed:a,memoryUsed:r,carbonBurnt:o,particlesUpdated:c,parameterValues:l,summaryMetrics:u}}catch{return null}}async function io(n){try{const e=await fetch(ao(n));if(!e.ok)return{};const t=await e.text(),s=Me(t);return oo(s)}catch{return{}}}function ao(n){return n.replace(/run_summary\.yaml($|\?)/,"parameters.yaml$1")}function Qe(n){const e=typeof n=="number"?n:Number(n);return Number.isFinite(e)?e:null}function ro(n){if(!n||typeof n!="object")return{};const e=n,t={};for(const[s,i]of Object.entries(e)){if(!i||typeof i!="object")continue;const a=i,r=typeof a.label=="string"?a.label:s,o=a.value;o!=null&&(t[s]={label:r,value:String(o)})}return t}function oo(n){if(!n||typeof n!="object")return{};const e={};for(const[t,s]of Object.entries(n)){const i=Qe(s);i!==null&&(e[t]=i)}return e}const mi="[UniverseEngine]",lo=["planetary","galaxy","cosmos"];function pi(){return fi(lo).verboseLogging}function ie(n,e){pi()&&console.info(mi,n,e??"")}function Ae(n,e){pi()&&console.warn(mi,n,e??"")}const co={local:"assets/local-manifest.json",online:Er};function uo(n="local"){let e=n;const t=new Map;return{getSource(){return e},setSource(s){s==="online"&&t.delete("online"),e=s,ie("Manifest source updated",{source:s})},resetCache(){t.clear()},async preloadActiveManifest(){await yi(e,t)},async findNearestVideo(s,i,a){const r=await ho(e,t,s,i,a);if(r)return r;const o=gi(s);return Ae("Falling back to placeholder assets",{simClassId:s,source:e,fallbackUrl:o}),{url:o,liveDataUrl:fo(s),summaryUrl:no(o)}}}}function gi(n){switch(n){case"planetary":return ee("assets/planet_test.mp4");case"galaxy":return ee("assets/galaxy_test.mp4");case"cosmos":return ee("assets/cosmo_test.mp4");default:return ee("assets/galaxy_test.mp4")}}function fo(n){switch(n){case"planetary":return ee("assets/planet_test_planetary_stats.csv");case"galaxy":return ee("assets/galaxy_test_galaxy_stats.csv");case"cosmos":return ee("assets/cosmo_test_cosmos_stats.csv");default:return ee("assets/galaxy_test_galaxy_stats.csv")}}async function yi(n,e){const t=e.get(n);if(t)return t;const s=co[n],i=fetch(ee(s)).then(async a=>{if(!a.ok)throw new Error(`Failed to load manifest: ${s}`);return ie("Loaded manifest",{source:n,manifestPath:s}),await a.json()}).catch(a=>(Ae("Manifest unavailable",{source:n,manifestPath:s,error:a instanceof Error?a.message:String(a)}),{version:1,runs:[]}));return e.set(n,i),i}async function ho(n,e,t,s,i){const r=(await yi(n,e)).runs.filter(d=>d.simulationId===t);if(r.length===0)return Ae("No manifest runs found for simulation",{simClassId:t,source:n}),null;let o=r[0],c=bs(o,s,i);for(const d of r.slice(1)){const p=bs(d,s,i);p<c&&(o=d,c=p)}const l=o.defaultView??Object.keys(o.views)[0],u=o.views[l];return u?(ie("Selected manifest-backed run",{simClassId:t,source:n,runId:o.runId,selectedValues:i,distance:c,viewId:l}),{url:ee(u),liveDataUrl:ee(o.liveDataPath),summaryUrl:ee(o.summaryPath),viewId:l,runId:o.runId,views:Object.fromEntries(Object.entries(o.views).map(([d,p])=>[d,ee(p)]))}):null}function bs(n,e,t){return e.length===0?0:e.reduce((i,a)=>{var l;const r=t[a.id]??a.fallbackValue,o=((l=n.parameters)==null?void 0:l[a.id])??a.fallbackValue,c=Math.max(a.max-a.min,1e-9);return i+Math.abs(r-o)/c},0)/e.length}const Rt={mode:"time",frames:[]};async function mo(n){const e=await fetch(n);if(!e.ok)throw new Error(`Failed to load live stats CSV: ${n}`);const t=await e.text();return yo(t)}function po(n,e,t=0){if(n.mode==="row")return bo(n.frames,e,t);const s=n.frames;if(s.length===0)return{};if(e<=s[0].t)return{...s[0].values};const i=s[s.length-1];if(e>=i.t)return{...i.values};const a=go(s,e),r=s[Math.max(0,a-1)],o=s[Math.min(s.length-1,a)],c=(e-r.t)/Math.max(o.t-r.t,1e-9);return wo(r.values,o.values,c)}function go(n,e){let t=1,s=n.length-1;for(;t<s;){const i=Math.floor((t+s)/2);n[i].t<=e?t=i+1:s=i}return t}function yo(n){const e=n.split(/\r?\n/).map(s=>s.trim()).filter(Boolean);if(e.length<2)return Rt;const t=yn(e[0]);return t[0]==="t"?{mode:"time",frames:e.slice(1).map(s=>{const i=yn(s),a={};for(let r=1;r<t.length;r+=1)a[t[r]]=i[r]??"";return{t:parseFloat(i[0]??"0")||0,values:a}})}:{mode:"row",frames:e.slice(1).map((s,i)=>{const a=yn(s),r={};for(let o=0;o<t.length;o+=1)r[t[o]]=a[o]??"";return{t:i,values:r}})}}function bo(n,e,t){if(n.length===0)return{};if(!Number.isFinite(t)||t<=0)return{...n[0].values};const s=Math.max(0,Math.min(1,e/t)),i=Math.round(s*(n.length-1));return{...n[i].values}}function yn(n){const e=[];let t="",s=!1;for(let i=0;i<n.length;i+=1){const a=n[i];if(a==='"'){s=!s;continue}if(a===","&&!s){e.push(t),t="";continue}t+=a}return e.push(t),e}function wo(n,e,t){const s=new Set([...Object.keys(n),...Object.keys(e)]),i={};for(const a of s){const r=n[a]??"",o=e[a]??r,c=parseFloat(r),l=parseFloat(o);if(Number.isFinite(c)&&Number.isFinite(l)){const u=c+(l-c)*t;i[a]=vo(u);continue}i[a]=t<.5?r:o}return i}function vo(n){return n.toFixed(2).replace(new RegExp("\\.0+$|(?<=\\..*?)0+$","g"),"").replace(/\.$/,"")}function _o(n){So(Ir,n)}function So(n,e){if(navigator.sendBeacon){const t=new Blob([JSON.stringify(e)],{type:"application/json"});if(navigator.sendBeacon(n,t)){ie("Run selection tracking dispatched",{simulationId:e.simulationId});return}}fetch(n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e),keepalive:!0}).then(t=>{t.ok?ie("Run selection tracked",{simulationId:e.simulationId}):Ae("Run selection tracking rejected",{simulationId:e.simulationId,status:t.status})}).catch(t=>{Ae("Run selection tracking failed",{simulationId:e.simulationId,error:t instanceof Error?t.message:String(t)})})}const ws=50*1024*1024,ko=8,Eo=6e3,Io=8e3,Lo=5e3,No=1200,To=100,bn={galaxy:"tron",planetary:"matrix",cosmos:"nostromo"};function Ao(n){const e=ze.map(h=>h.id);let t=fi(e),s=zn(t);const i=uo(t.manifestSource);t.manifestSource==="online"&&i.preloadActiveManifest();let a=Qn(t.lockedScaleId)??s[0]??ze[0],r=t.lockedScaleId?bn[a.id]:sr(),o=!1,c=null,l=null,u=0,d=Rt,p=!1;const m=Object.fromEntries(ze.map(h=>[h.id,Ti(h)]));fn(r);const b=gi(a.id),f=rr(n,b),g=document.createElement("div");g.className="display-chrome",g.classList.add("is-hidden"),n.appendChild(g);const y=document.createElement("div");y.className="orientation-overlay",y.innerHTML=`
  ...
  `,n.appendChild(k);const I=document.createElement("div");I.className="synth-logo is-hidden",I.innerHTML=`
    <img
      ...
      decoding="async"
    />
  `,n.appendChild(I);const L=document.createElement("div");L.className="display-chrome__top-left is-hidden",n.appendChild(L);const _=zr(L,{onHome(){en()},onViewSelected(h){if(h==="credits"){Lt("credits");return}Lt(h)},showHome:!t.lockedScaleId}),S=document.createElement("div");S.className="display-chrome__left-center",g.appendChild(S);const N=Br(S,{onSelect(h){Jn(h)},onInfo(h,w,M){A.textContent=w,F.textContent=M,v.classList.add("is-visible")}}),v=document.createElement("div");v.className="view-info-overlay",v.innerHTML=`
